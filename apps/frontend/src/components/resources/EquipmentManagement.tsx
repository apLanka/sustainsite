...
export default function EquipmentManagement() {
    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            const res = await resourcesApi.createEquipment({ ...payload, currentProjectId: activeProjectId } as CreateEquipmentPayload & {
    ...
    return (<div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex items-center justify-between">
        ...
        {canMutate && (<button onClick={() => setShowCreateModal(true)} className="bg-primary text-white px-6 py-3 rounded-2xl text-xs font-bold hover:bg-primary-dark transition-all flex items-center gap-2 shadow-lg shadow-primary/10">
          <span className="material-symbols-outlined text-sm">add</span> Register Asset
        </button>)}
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {equipment.map(item => (<div key={item._id} className="bg-white border border-slate-100 rounded-[32px] p-8 shadow-sm hover:-translate-y-1 transition-all group relative overflow-hidden">
            ...
            {(canMutate || canDel) && (<div className="flex items-center gap-2 flex-wrap">
              {canMutate && (<button onClick={() => openEdit(item)} className="flex-1 py-2 text-[10px] font-black uppercase tracking-wider bg-slate-50 text-slate-600 rounded-xl hover:bg-primary hover:text-white transition-all flex items-center justify-center gap-1">
                <span className="material-symbols-outlined text-sm">edit</span> Edit
              </button>)}
              {canMutate && (<button onClick={() => { setShowMaintenanceModal(item); }} className="flex-1 py-2 text-[10px] font-black uppercase tracking-wider bg-amber-50 text-amber-700 rounded-xl hover:bg-amber-100 transition-all flex items-center justify-center gap-1">
                <span className="material-symbols-outlined text-sm">build</span> Service
              </button>)}
              {canMutate && item.status === 'Available' && activeProjectId && item.currentProjectId !== activeProjectId && (<button onClick={() => handleAssign(item)} className="flex-1 py-2 text-[10px] font-black uppercase tracking-wider bg-emerald-50 text-emerald-700 rounded-xl hover:bg-emerald-100 transition-all flex items-center justify-center gap-1">
                  <span className="material-symbols-outlined text-sm">link</span> Assign
                </button>)}
              {canDel && (<button onClick={() => setDeleteConfirm(item._id)} className="w-9 h-9 rounded-xl bg-rose-50 text-rose-500 hover:bg-rose-100 transition-all flex items-center justify-center">
                <span className="material-symbols-outlined text-sm">delete</span>
              </button>)}
            </div>)}
          </div>))}

        {canMutate && (<button onClick={() => setShowCreateModal(true)} className="border-2 border-dashed border-slate-200 rounded-[32px] p-8 flex flex-col items-center justify-center gap-4 group hover:border-primary/30 hover:bg-slate-50/50 transition-all">
      ...
      {editingEquipment && (<div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
      ...
      {showMaintenanceModal && (<div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
