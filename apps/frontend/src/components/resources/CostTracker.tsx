...
export default function CostTracker() {
    ...
    if (error || !data) {
        return (<div className="flex items-center justify-center h-64 text-rose-500">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                        <div>
                            ...
                            <h3 className="text-4xl font-black text-primary tracking-tighter">
                                ...
                                <span className="text-slate-300 text-lg font-bold ml-2">/ ${formatCurrency(budget)}</span>
                            </h3>
                        </div>
                        <div className={`flex items-center gap-3 px-4 py-2 rounded-2xl border ring-4 ring-emerald-50/50 ${isOnTrack ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-rose-50 text-rose-600 border-rose-100'}`}>
                            <span className="material-symbols-outlined text-[18px] font-bold">{isOnTrack ? 'verified' : 'warning'}</span>
                            <span className="text-xs font-black uppercase tracking-widest">{isOnTrack ? 'On Track' : 'Over Budget'}</span>
                        </div>
                    </div>

                    <div className="space-y-3">
                        <div className="relative h-5 bg-slate-100 rounded-full overflow-hidden shadow-inner ring-1 ring-slate-100">
                            <div className="absolute top-0 left-0 h-full bg-gradient-to-r from-primary to-emerald-500 rounded-full transition-all duration-1000 ease-out flex items-center justify-end px-2" style={{ width: `${percentage}%` }}>
                ...
                <div className="bg-white border border-slate-100 rounded-[32px] p-8 shadow-sm flex flex-col justify-between">
                    <div>
                        ...
                        <div className="space-y-5">
                            {allocationMix.length > 0 ? (allocationMix.map((item, idx) => {
            const colors = ['bg-primary', 'bg-sky-500', 'bg-amber-500', 'bg-rose-500', 'bg-purple-500'];
            return (<div key={item.category} className="group cursor-help">
                                            <div className="flex justify-between items-center mb-2">
                                                <div className="flex items-center gap-2">
                                                    <div className={`w-2 h-2 rounded-full ${colors[idx % colors.length]} group-hover:scale-125 transition-transform`}/>
                                                    <span className="text-primary text-[10px] font-black uppercase tracking-widest">{item.category}</span>
                                                </div>
                                                <span className="text-primary font-black text-xs tabular-nums">{safeNumber(item.percentage).toFixed(1)}%</span>
                                            </div>
                                            <div className="h-1 bg-slate-50 rounded-full overflow-hidden">
                                                <div className={`h-full ${colors[idx % colors.length]} opacity-20 group-hover:opacity-100 transition-all duration-500`} style={{ width: `${Math.min(100, safeNumber(item.percentage))}%` }}/>
                                            </div>
                                        </div>);
