...
import type { CreateMaterialPayload, MaterialAsset, MaterialCategory, MaterialSupplierRef, Supplier, UpdateMaterialPayload, } from '@/types/resources';
...
export default function MaterialInventory() {
    ...
    return (<div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      {hasLowStock && (<div className="bg-rose-50 border border-rose-100 p-4 rounded-xl flex items-center gap-3 text-rose-700">
      ...
      <div className="bg-white border border-slate-100 rounded-3xl overflow-hidden shadow-sm">
        <div className="p-6 border-b border-slate-100 flex items-center justify-between">
          ...
          {canCreateEdit && (<button onClick={() => setShowCreateModal(true)} className="bg-secondary text-white px-4 py-2 rounded-xl text-xs font-bold hover:bg-secondary-dark transition-all flex items-center gap-2 shadow-sm">
            <span className="material-symbols-outlined text-sm">add</span> New Material
          </button>)}
        </div>
        <div className="overflow-x-auto">
          {materials.length === 0 ? (<div className="py-12 text-center">
              ...
              <thead>
                <tr className="bg-slate-50/50 text-slate-400 text-[10px] font-black uppercase tracking-widest">
                  ...
                  {(canCreateEdit || canDeleteMat) && (<th className="px-6 py-4">Actions</th>)}
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 text-sm">
                {materials.map(m => (<tr key={m._id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-5">
                      <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest ${m.status === 'In Stock' ? 'bg-emerald-100 text-emerald-700' :
                    m.status === 'In Transit' ? 'bg-sky-100 text-sky-700' :
                        m.status === 'Delivered' ? 'bg-blue-100 text-blue-700' :
                            m.status === 'Ordered' ? 'bg-amber-100 text-amber-700' :
                                m.status === 'Used' ? 'bg-slate-100 text-slate-600' : 'bg-rose-100 text-rose-700'}`}>{m.status}</span>
                    </td>
                    <td className="px-6 py-5 text-slate-500 font-medium text-xs">{supplierName(m.supplier)}</td>
                    {(canCreateEdit || canDeleteMat) && (<td className="px-6 py-5">
                      <div className="flex items-center gap-2">
                        {canCreateEdit && (<button onClick={() => openEdit(m)} className="w-8 h-8 rounded-lg bg-slate-50 text-slate-400 hover:bg-primary hover:text-white transition-all flex items-center justify-center" title="Edit">
                          <span className="material-symbols-outlined text-sm">edit</span>
                        </button>)}
                        {canDeleteMat && (<button onClick={() => setDeleteConfirm(m._id)} className="w-8 h-8 rounded-lg bg-slate-50 text-slate-400 hover:bg-rose-500 hover:text-white transition-all flex items-center justify-center" title="Delete">
          ...
          <div className="bg-white rounded-[40px] w-full max-w-lg relative overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-300">
            ...
            <div className="p-10 space-y-5 max-h-[70vh] overflow-y-auto">
              ...
              <div className="space-y-1.5">
                ...
                {suppliers.length > 0 ? (<select className="input-standard w-full h-12 cursor-pointer" value={form.supplier || ''} onChange={e => setForm(f => ({ ...f, supplier: e.target.value }))}>
                    <option value="">Select supplier...</option>
                    {suppliers.map(s => <option key={s._id} value={s._id}>{s.companyName}</option>)}
                  </select>) : (<input type="text" placeholder="Supplier ID" className="input-standard w-full h-12" value={form.supplier || ''} onChange={e => setForm(f => ({ ...f, supplier: e.target.value }))}/>)}
              </div>
              <div className="space-y-1.5">
      ...
      {editingMaterial && (<div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
