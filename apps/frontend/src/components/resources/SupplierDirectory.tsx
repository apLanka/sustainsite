...
export default function SupplierDirectory() {
  ...
  const filteredSuppliers = suppliers.filter((s) => {
    ...
    if (
      filters.material &&
      !s.materialsSupplied.some((m) => m.toLowerCase().includes(filters.material.toLowerCase()))
    )
      return false;
    return true;
  });
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-xs font-bold mt-1">
            Verified sustainable supply chain network
          </p>
        </div>
        <div className="flex gap-3">
          <button
            onClick={() => setShowFilters((v) => !v)}
            className={`w-12 h-12 border rounded-2xl transition-all flex items-center justify-center ${showFilters ? 'bg-primary text-white border-primary' : 'border-slate-100 text-slate-400 hover:bg-slate-50'}`}
          >
        ...
        <div className="bg-slate-50 border border-slate-100 rounded-2xl p-5 flex flex-wrap gap-4 items-center">
          <label className="flex items-center gap-2 text-xs font-bold text-slate-600 cursor-pointer">
            <input
              ...
              onChange={(e) => setFilters((f) => ({ ...f, certified: e.target.checked }))}
            />
          ...
          <input
            ...
            className="input-standard h-9 text-xs px-3 min-w-[160px]"
            value={filters.material}
            onChange={(e) => setFilters((f) => ({ ...f, material: e.target.value }))}
          />
          {(filters.certified || filters.material) && (
            <button
              onClick={() => setFilters({ certified: false, material: '' })}
      ...
      {filteredSuppliers.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {filteredSuppliers.map((supplier) => (
            <div
              key={supplier._id}
              className="bg-white border border-slate-100 rounded-[32px] p-8 shadow-sm transition-all group relative overflow-hidden"
            >
              {supplier.isSustainabilityCertified && (
                <div className="absolute top-0 right-0 p-4">
                  <div className="flex items-center gap-1.5 bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border border-emerald-200 shadow-sm">
                    <span className="material-symbols-outlined text-[14px]">eco</span> Green
                    Certified
                  </div>
                </div>
              )}
              <div className="flex items-start gap-6 mb-8">
                ...
                <div className="pt-2 min-w-0">
                  <h4 className="text-primary font-black text-2xl tracking-tight mb-1 truncate">
              ...
              <div className="space-y-4 mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <button
                    onClick={() => copyEmail(supplier.email)}
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          <div
            ...
            onClick={closeCreatePartnerModal}
          />
            ...
            >
                      ...
                      >
                        ...
                        <button
                          ...
                          onClick={() =>
                            setForm((f) => ({
                              ...f,
                              materialsSupplied: f.materialsSupplied?.filter((x) => x !== m),
                            }))
                          }
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, isSustainabilityCertified: e.target.checked }))
                  }
                />
                <label
                  htmlFor="isCert"
              ...
              <div className="flex flex-wrap gap-4">
                <label className="flex items-center gap-2 text-xs font-bold text-slate-600 cursor-pointer">
                  <input
                    type="checkbox"
                    className="w-4 h-4 accent-primary"
                    checked={editForm.isSustainabilityCertified ?? false}
                    onChange={(e) =>
                      setEditForm((f) => ({ ...f, isSustainabilityCertified: e.target.checked }))
                    }
                  />
                  Sustainability Certified
                </label>
