...
export default function SupplierDirectory() {
    ...
    const filteredSuppliers = suppliers.filter(s => {
        if (filters.certified && !s.isSustainabilityCertified)
            return false;
        if (filters.material && !s.materialsSupplied.some(m => m.toLowerCase().includes(filters.material.toLowerCase())))
            return false;
        return true;
    });
    const handleAddMaterial = () => {
        const t = materialsInput.trim();
        if (t && !form.materialsSupplied?.includes(t))
            setForm(f => ({ ...f, materialsSupplied: [...(f.materialsSupplied || []), t] }));
        setMaterialsInput('');
    };
    const handleCreate = async () => {
        ...
        try {
            ...
            const msg = (err as {
                ...
                message?: string;
            })?.response?.data?.message ?? 'Failed to add partner';
            toast.error(msg);
        }
        finally {
            setIsCreating(false);
    ...
    return (<div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          ...
          {canMutate && (<button onClick={openCreatePartnerModal} className="bg-primary text-white px-6 py-3 rounded-2xl text-xs font-bold hover:bg-primary-dark transition-all flex items-center gap-2 shadow-lg shadow-primary/10">
      ...
      {showFilters && (<div className="bg-slate-50 border border-slate-100 rounded-2xl p-5 flex flex-wrap gap-4 items-center">
          ...
          {(filters.certified || filters.material) && (<button onClick={() => setFilters({ certified: false, material: '' })} className="text-xs font-bold text-slate-400 hover:text-primary transition-colors flex items-center gap-1">
              <span className="material-symbols-outlined text-sm">close</span> Clear
            </button>)}
        </div>)}

      {filteredSuppliers.length === 0 ? (<div className="py-16 text-center">
          ...
          {suppliers.length === 0 && canMutate && (<button onClick={openCreatePartnerModal} className="mt-4 text-xs font-bold text-emerald-600 hover:underline">
              Add your first partner →
            </button>)}
        </div>) : (<div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {filteredSuppliers.map(supplier => (<div key={supplier._id} className="bg-white border border-slate-100 rounded-[32px] p-8 shadow-sm transition-all group relative overflow-hidden">
              {supplier.isSustainabilityCertified && (<div className="absolute top-0 right-0 p-4">
              ...
              <div className="space-y-4 mb-6">
                ...
                {(supplier.address?.city || supplier.address?.country) && (<div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 flex-shrink-0">
              ...
              {(canMutate || canDel) && (<div className="flex items-center gap-2 pt-4 border-t border-slate-50">
                {canMutate && (<button onClick={() => openEdit(supplier)} className="flex-1 py-2 text-[10px] font-black uppercase tracking-wider bg-slate-50 text-slate-600 rounded-xl hover:bg-primary hover:text-white transition-all flex items-center justify-center gap-1">
                  <span className="material-symbols-outlined text-sm">edit</span> Edit
                </button>)}
                {canDel && (<button onClick={() => setDeleteConfirm(supplier._id)} className="w-9 h-9 rounded-xl bg-rose-50 text-rose-500 hover:bg-rose-100 transition-all flex items-center justify-center">
      ...
      {showCreateModal && (<div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <div className="bg-white rounded-[40px] w-full max-w-lg relative overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-300">
            ...
            <form className="p-10 space-y-5 max-h-[70vh] overflow-y-auto" autoComplete="off" onSubmit={(e) => {
                e.preventDefault();
                void handleCreate();
            }}>
              ...
              <div className="space-y-1.5">
                ...
                {(form.materialsSupplied ?? []).length > 0 && (<div className="flex flex-wrap gap-2 mt-2">
                    {form.materialsSupplied!.map(m => (<span key={m} className="px-3 py-1 bg-emerald-100 text-emerald-700 rounded-full text-xs font-bold flex items-center gap-1">
                        {m}
      ...
      {editingSupplier && (<div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
      ...
      {deleteConfirm && (<div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
