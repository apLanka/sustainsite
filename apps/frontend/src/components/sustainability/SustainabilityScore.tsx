...
const SustainabilityScore = ({ score = 0, trend, scoreCategory }: SustainabilityScoreProps) => {
    ...
    const rankIcon = scoreCategory === 'Green'
        ...
        : scoreCategory === 'Yellow'
            ? 'star_half'
            : 'warning';
    return (<div className="flex flex-col items-center justify-center p-8 bg-emerald-950 rounded-3xl shadow-xl shadow-emerald-950/20 relative overflow-hidden group h-full">
      ...
      <div className="relative w-56 h-56 flex items-center justify-center">
        <svg className="transform -rotate-90 w-full h-full">
          ...
          <circle className="text-emerald-900" strokeWidth="12" stroke="currentColor" fill="transparent" r={radius} cx="112" cy="112"/>
          
          <motion.circle className="text-secondary" strokeWidth="12" strokeDasharray={circumference} initial={{ strokeDashoffset: circumference }} animate={{ strokeDashoffset }} transition={{ duration: 1.5, ease: "easeOut" }} strokeLinecap="round" stroke="currentColor" fill="transparent" r={radius} cx="112" cy="112"/>
        </svg>
        
        
        <div className="absolute flex flex-col items-center justify-center select-none">
          <motion.h4 className="text-6xl font-black text-white tracking-tighter" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1, duration: 0.5 }}>
            {score}
          </motion.h4>
          <motion.p className="text-[10px] font-black uppercase tracking-widest text-emerald-400 mt-1" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5, duration: 0.5 }}>
            Global Score
          </motion.p>
        </div>
      </div>
