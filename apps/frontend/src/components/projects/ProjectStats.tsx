...
const ProjectStats = ({ project }: Props) => {
  ...
  const completionPct =
    milestones.length > 0 ? calcMilestoneProgress(milestones) : project.completionPercentage;
  const budgetUsedPct =
    project.budget > 0 ? Math.min(100, Math.round((project.actualCost / project.budget) * 100)) : 0;
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
      ...
      <div className="bg-surface-container-lowest p-6 rounded-2xl shadow-sm border border-slate-100/50 transition-all group">
        ...
        <div className="flex items-end gap-2">
          ...
          <span
            className="material-symbols-outlined text-emerald-600 text-lg mb-1"
        ...
        <div className="h-1.5 w-full bg-slate-100 rounded-full mt-4 overflow-hidden">
          <div
            ...
            style={{ width: `${project.sustainabilityScore}%` }}
          />
        </div>
      </div>
