    ...
    <div className="space-y-3">
      ...
      <div className="relative">
        ...
        <input
          ...
          className={`input-standard w-full pl-11 h-12 ${error ? 'border-rose-300' : ''}`}
          value={query}
          onChange={handleQueryChange}
          autoComplete="off"
        />
        {isSearching && (
          <span className="material-symbols-outlined absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 animate-spin text-lg">
            progress_activity
          </span>
        )}
        ...
        {suggestions.length > 0 && (
          <ul className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-100 rounded-2xl shadow-xl overflow-hidden z-50">
            {suggestions.map((s) => (
              <li
                key={s.id}
                className="px-5 py-3 text-sm font-medium text-primary hover:bg-slate-50 cursor-pointer transition-colors flex items-center gap-3 border-b border-slate-50 last:border-0"
      ...
      {value?.latitude && value?.longitude && (
        <div className="flex items-center gap-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
          ...
          <button
            ...
            onClick={() => { setQuery(''); onChange({ address: '', latitude: 0, longitude: 0 }); }}
          >
            Clear
          </button>
        </div>
