...
const fmtBudget = (n: number) => n >= 1000000
...
const ProjectCard = ({ project }: {
    ...
    return (<div className="relative bg-white rounded-2xl border border-slate-100 shadow-sm group transition-all duration-300 hover:-translate-y-0.5 flex flex-col cursor-pointer">
      ...
      <div className="px-5 pt-3 pb-0 space-y-1">
        <Link to={`/projects/${project._id}`} className="after:absolute after:inset-0 after:rounded-2xl after:content-[''] focus:outline-none focus-visible:after:ring-2 focus-visible:after:ring-secondary/50" aria-label={`View project: ${project.projectName}`}>
      ...
      <div className="px-5 pt-3">
        <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-secondary to-emerald-400 rounded-full transition-all duration-1000" style={{ width: `${project.completionPercentage}%` }}/>
        </div>
      </div>
      ...
      <div className="px-5 pt-3 flex flex-wrap gap-x-4 gap-y-1.5">
        ...
        {project.currentPhase && (<div className="flex items-center gap-1 text-slate-400">
      ...
      <div className="px-5 pt-3 flex items-center gap-3">
        ...
        {project.actualCost > 0 && (<div className={`flex items-center gap-1 text-[10px] font-bold ${budgetOver ? 'text-rose-500' : 'text-emerald-600'}`}>
      ...
      <div className="px-5 py-4 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 min-w-0">
          ...
          {project.teamMembers?.length > 0 && (<div className="flex -space-x-1.5 ml-1">
              {project.teamMembers.slice(0, 3).map((m) => (<div key={m._id} title={`${m.firstName} ${m.lastName}`} className="w-5 h-5 rounded-full border border-white bg-secondary/20 flex items-center justify-center text-[7px] font-black text-secondary">
                  {initials(m)}
                </div>))}
              {project.teamMembers.length > 3 && (<div className="w-5 h-5 rounded-full bg-slate-100 border border-white flex items-center justify-center text-[7px] font-bold text-slate-500">
...
const SkeletonCard = () => (<div className="bg-white rounded-2xl border border-slate-100 shadow-sm animate-pulse flex flex-col">
...
const ProjectsGrid = () => {
    ...
    return (<div className="space-y-8">
      ...
      {pagination && pagination.totalPages > 1 && (<div className="flex justify-center items-center gap-3 pt-4">
          <button className="px-4 py-2 text-xs font-bold text-slate-500 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed transition-all" disabled={filters.page <= 1} onClick={() => useProjectStore.getState().setFilters({ page: filters.page - 1 })}>
            Previous
          </button>
          <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">
            {filters.page} / {pagination.totalPages}
          </span>
          <button className="px-4 py-2 text-xs font-bold text-slate-500 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed transition-all" disabled={filters.page >= pagination.totalPages} onClick={() => useProjectStore.getState().setFilters({ page: filters.page + 1 })}>
            Next
          </button>
        </div>)}
    </div>);
