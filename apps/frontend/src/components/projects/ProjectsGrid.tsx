...
const ProjectCard = ({ project }: { project: Project }) => {
  ...
  ) => (p?.firstName && p?.lastName ? `${p.firstName[0]}${p.lastName[0]}`.toUpperCase() : '?');
  ...
  return (
    <div className="relative bg-white rounded-2xl border border-slate-100 shadow-sm group transition-all duration-300 hover:-translate-y-0.5 flex flex-col cursor-pointer">
      <div className="flex items-center justify-between px-5 pt-5 pb-0">
        <span
          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${sc.bg} ${sc.text}`}
        >
          <span
            className={`w-1.5 h-1.5 rounded-full ${sc.dot} ${project.status === ProjectStatus.IN_PROGRESS ? 'animate-pulse' : ''}`}
          />
          {project.status}
        </span>
      ...
      <div className="px-5 pt-3 flex items-center gap-3">
        <div className="flex items-center gap-1.5 bg-slate-50 rounded-lg px-2.5 py-1.5">
          ...
          <span className="text-[10px] font-black text-slate-600 tracking-wide">
            {fmtBudget(project.budget)}
          </span>
        </div>
        {project.actualCost > 0 && (
          <div
            className={`flex items-center gap-1 text-[10px] font-bold ${budgetOver ? 'text-rose-500' : 'text-emerald-600'}`}
          >
            <span className="material-symbols-outlined !text-[13px]">
              {budgetOver ? 'trending_up' : 'trending_down'}
            </span>
            <span>
              {budgetOver ? 'Over' : 'Under'} by {fmtBudget(Math.abs(project.budgetVariance))}
            </span>
          </div>
        )}
      </div>
      ...
      <div className="px-5 py-4 flex items-center justify-between gap-3">
        ...
        <div className="flex flex-col items-end shrink-0">
          ...
          <span
            className={`text-[10px] font-bold tabular-nums ${project.daysRemaining <= 7 && project.daysRemaining >= 0 ? 'text-rose-500' : 'text-slate-500'}`}
          >
            {project.daysRemaining > 0
              ? `${project.daysRemaining}d · ${fmt(project.endDate)}`
              : `Ended ${fmt(project.endDate)}`}
          </span>
        </div>
      </div>
    </div>
