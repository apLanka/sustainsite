...
const MilestoneTimeline = ({ milestones, onEdit }: Props) => {
  ...
  return (
    <div className="relative pl-10 md:pl-12 lg:pl-16 pb-20">
      ...
      <div
        className="absolute left-[19px] top-0 w-1 bg-gradient-to-b from-emerald-600 via-secondary to-emerald-400 rounded-full shadow-[0_0_15px_rgba(5,150,105,0.4)] z-0 transition-all duration-700"
        style={{ height: `${stemPct}%` }}
      />
        ...
        {milestones.map((milestone) => {
          ...
          return (
            <div key={milestone._id} className="relative group">
              <div
                className={`absolute -left-[20px] top-5 w-10 h-0.5 transition-colors duration-500 ${isCompleted || isActive ? 'bg-emerald-500/30' : 'bg-slate-100'}`}
              />

              <div
                className={`absolute -left-[30px] top-0 w-6 h-6 rounded-full border-2 border-white flex items-center justify-center z-10 transition-all duration-500 shadow-sm ${
                  isCompleted
                    ? 'bg-emerald-600 scale-110'
                    : isActive
                      ? 'bg-secondary ring-2 ring-emerald-500/20'
                      : 'bg-slate-200'
                }`}
              >
                {isCompleted && (
                  <span className="material-symbols-outlined text-white !text-xs font-bold leading-none">
                    check
                  </span>
                )}
              ...
              >
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                  <div className="flex-1 min-w-0">
                    <span
                      className={`text-[9px] font-black uppercase tracking-[0.2em] mb-1 block ${isActive ? 'text-secondary-container' : 'text-slate-400'}`}
                    >
                      {milestone.status}
                    </span>
                    <h4
                      className={`text-xl font-bold tracking-tight font-headline ${isActive ? 'text-white' : 'text-primary'}`}
                    >
                      {milestone.title}
                    </h4>
                    {milestone.description && (
                      <p
                        className={`text-xs mt-1 font-medium ${isActive ? 'text-emerald-300/70' : 'text-slate-400'}`}
                      >
                        {milestone.description}
                      </p>
                    )}
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    <div
                      className={`px-4 py-2 rounded-xl text-[10px] font-black tracking-widest uppercase font-headline border ${
                        isActive
                ...
                <div className="flex flex-wrap items-center justify-between gap-6">
                  <div className="flex items-center gap-3">
                    <div>
                      <p
                        className={`text-[8px] font-bold uppercase tracking-widest leading-none ${isActive ? 'text-emerald-400/70' : 'text-slate-400'}`}
                      >
                        Completion
                      </p>
                      <p
                        className={`text-xs font-bold mt-1 ${isActive ? 'text-emerald-50' : 'text-primary'}`}
                      >
