  ...
  return (
    <section className="bg-surface-container-lowest rounded-2xl shadow-sm border border-slate-100/50 overflow-hidden">
      <div className="p-8 border-b border-slate-50 flex justify-between items-center">
        <h3 className="text-xl font-extrabold text-primary tracking-tighter font-headline">
      ...
      <div className="p-8 space-y-8">
        {isLoading ? (
          ...
          <div className="text-center py-16">
            ...
            <Link
              to="/projects/new"
              className="mt-4 inline-block text-xs font-bold text-secondary hover:underline"
            >
            ...
            return (
              <div key={project._id} className="relative group">
                <div className="flex justify-between items-start mb-3">
                  <div className="min-w-0 flex-1 pr-4">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span
                        className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest shrink-0 ${statusStyle.bg} ${statusStyle.text}`}
                      >
                        {project.status}
                      </span>
                    </div>
                  ...
                  <span
                    className={`text-2xl font-black tracking-tighter shrink-0 ${isFinal ? 'text-amber-700' : 'text-secondary'}`}
                  >
                    {score}%
                  </span>
                </div>
                ...
                <div className="h-3 w-full bg-slate-100 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-1000 bg-gradient-to-r ${isFinal ? 'from-amber-600 to-amber-700' : 'from-secondary to-primary-container'}`}
                    style={{ width: `${score}%` }}
                  />
                </div>
