...
const Initials = ({ name }: {
    ...
    const initials = parts.length >= 2
        ...
        : parts[0].slice(0, 2);
    return (<div className="w-7 h-7 rounded-full bg-secondary/10 text-secondary text-[10px] font-black flex items-center justify-center border-2 border-white uppercase">
      {initials}
    </div>);
};
const SkeletonRow = () => (<div className="animate-pulse space-y-3">
...
const ProjectOverview = ({ isLoading }: {
    isLoading: boolean;
}) => {
    const { projects } = useDashboardStore();
    return (<section className="bg-surface-container-lowest rounded-2xl shadow-sm border border-slate-100/50 overflow-hidden">
      <div className="p-8 border-b border-slate-50 flex justify-between items-center">
        ...
        <Link to="/projects" className="text-xs font-bold text-secondary hover:underline uppercase tracking-widest cursor-pointer font-headline">
          View All Projects
        </Link>
      </div>

      <div className="p-8 space-y-8">
        {isLoading ? (Array.from({ length: 3 }).map((_, i) => <SkeletonRow key={i}/>)) : projects.length === 0 ? (<div className="text-center py-16">
            ...
            const managerName = project.projectManager
                ? typeof project.projectManager === 'string'
                    ...
                    : `${(project.projectManager as {
                        firstName?: string;
                        lastName?: string;
                    }).firstName ?? ''} ${(project.projectManager as {
                        firstName?: string;
                        lastName?: string;
                    }).lastName ?? ''}`.trim()
                : null;
            return (<div key={project._id} className="relative group">
                ...
                <div className="h-3 w-full bg-slate-100 rounded-full overflow-hidden">
                  <div className={`h-full rounded-full transition-all duration-1000 bg-gradient-to-r ${isFinal ? 'from-amber-600 to-amber-700' : 'from-secondary to-primary-container'}`} style={{ width: `${score}%` }}/>
                </div>

                <div className="flex justify-between items-center mt-3">
                  <div className="flex items-center gap-2">
                    {managerName && <Initials name={managerName}/>}
                    {managerName && (<span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">{managerName}</span>)}
                  </div>
                  <div className="flex items-center gap-3">
                    {project.endDate && (<span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
                        ETA: {new Date(project.endDate).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}
                      </span>)}
                    <Link to={`/projects/${project._id}`} className="text-[10px] font-black uppercase tracking-widest text-secondary hover:underline">
                      View
                    </Link>
                  </div>
                </div>
