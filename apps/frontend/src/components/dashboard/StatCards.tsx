...
const StatCards = ({ isLoading }: { isLoading: boolean }) => {
  const { activeCount, avgSustainability, pendingApprovals, highRiskCount, projects } =
    useDashboardStore();
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
      <div className="bg-surface-container-lowest p-6 rounded-xl shadow-sm border border-slate-100/50 flex flex-col justify-between group transition-all">
        ...
        <div>
          ...
          <p className="text-sm font-semibold text-slate-500 mt-1 uppercase tracking-wider font-headline">
            In Progress
          </p>
        </div>
      </div>
      ...
      <div className="bg-surface-container-lowest p-6 rounded-xl shadow-sm border border-slate-100/50 flex flex-col items-center justify-center text-center group transition-all">
            ...
            {isLoading ? (
              ...
              <>
                ...
                <span className="text-[9px] font-bold text-secondary uppercase tracking-widest mt-1">
                  {avgSustainability >= 80
                    ? 'Excellent'
                    : avgSustainability >= 50
                      ? 'Good'
                      : 'Needs Work'}
                </span>
              </>
        ...
        <h3 className="text-sm font-semibold text-slate-500 uppercase tracking-wider font-headline">
      ...
      <div className="bg-surface-container-lowest p-6 rounded-xl shadow-sm border border-slate-100/50 flex flex-col justify-between group transition-all">
        ...
        {!isLoading && pendingApprovals > 0 && (
          <Link
            to={
              projects.length === 1
                ? `/projects/${firstProjectId}/documents?status=Under Review`
                : '/projects'
            }
      ...
      <div className="bg-surface-container-lowest p-6 rounded-xl shadow-sm border border-slate-100/50 flex flex-col justify-between group transition-all">
        <div className="flex justify-between items-start mb-4">
          <div
            className={`w-14 h-14 rounded-xl flex items-center justify-center ${highRiskCount > 0 ? 'bg-rose-50 text-rose-600' : 'bg-emerald-50 text-secondary'}`}
          >
            <span
              className="material-symbols-outlined !text-3xl"
              style={{ fontVariationSettings: "'FILL' 1" }}
        ...
        <div>
          {isLoading ? (
            ...
            <h3
              className={`text-3xl font-extrabold tracking-tighter ${highRiskCount > 0 ? 'text-rose-600' : 'text-primary'}`}
            >
              {highRiskCount}
            </h3>
          )}
        ...
        {!isLoading && (
          <p
            className={`mt-4 text-xs font-bold uppercase tracking-widest ${highRiskCount > 0 ? 'text-rose-500' : 'text-emerald-600'}`}
          >
