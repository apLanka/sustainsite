...
const Sidebar = () => {
  ...
  `;
    return (<aside className="h-screen w-64 fixed left-0 top-0 overflow-y-auto bg-primary flex flex-col py-6 shadow-xl shadow-primary/20 z-50">
      <div className="px-6 mb-8">
        <Logo size="lg" variant="dark"/>
      </div>
      
      <nav className="flex-1 px-4 space-y-1">
        {navItems.map((item) => (<NavLink key={item.label} to={item.to} className={linkClass}>
        ...
        {adminItems.length > 0 && (<>
            ...
            {adminItems.map((item) => (<NavLink key={item.label} to={item.to} className={linkClass}>
                <span className="material-symbols-outlined">{item.icon}</span>
                <span>{item.label}</span>
              </NavLink>))}
          </>)}
      ...
      {showCreateProject && (<div className="px-6 py-4 mt-auto">
          <Link to="/projects/new" className="w-full py-3 px-4 bg-gradient-to-br from-secondary to-primary-container text-white rounded-lg font-bold text-sm shadow-lg shadow-primary/40 flex items-center justify-center gap-2 hover:brightness-110 active:scale-[0.98] transition-all cursor-pointer no-underline">
      ...
      <div className="px-4 border-t border-white/5 pt-4 pb-2 space-y-1">
        {bottomItems.map((item) => (<NavLink key={item.label} to={item.to} className={linkClass}>
