...
const ProjectMenu = () => {
  ...
  const handleStatusChange = async (status: ProjectStatus) => {
    if (!id || status === selectedProject?.status) { setOpen(false); return; }
    setBusy(true);
    try {
          ...
          <div>
            ...
            <h2 className="text-4xl font-extrabold text-primary tracking-tighter leading-none font-headline flex items-center gap-4 flex-wrap">
              ...
              {selectedProject && (() => {
                ...
                return (
                  <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border transition-colors duration-300 ${sc.bg} ${sc.text} border-current/10`}>
