...
const ProjectMenu = () => {
    ...
    return (<div ref={menuRef} className="relative">
      ...
      <button type="button" onClick={() => { setOpen((v) => !v); setConfirming(false); }} disabled={busy || !selectedProject} className="w-12 h-12 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-center text-primary hover:bg-slate-100 transition-all cursor-pointer shadow-sm disabled:opacity-60" aria-label="Project options">
        {busy
      ...
      {open && (<div className="absolute right-0 top-14 w-56 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-150">

          {!confirming ? (<>
              {canStatus && (<div className="px-3 pt-3 pb-1">
                ...
                {statusOptions.map(({ value, label, icon }) => {
                        ...
                        return (<button key={value} type="button" onClick={() => handleStatusChange(value)} className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-bold transition-all text-left
                        ${active
                                ? 'bg-emerald-50 text-emerald-700'
                                : 'text-slate-600 hover:bg-slate-50 hover:text-primary'}`}>
                      <span className="material-symbols-outlined text-[15px]">{icon}</span>
                      {label}
                      {active && (<span className="ml-auto material-symbols-outlined text-[14px] text-emerald-500">
                          check
                        </span>)}
                    </button>);
                    })}
              ...
              {canDel && (<div className={`px-3 ${canStatus ? 'pb-3' : 'pt-3 pb-3'}`}>
                <button type="button" onClick={() => setConfirming(true)} className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-bold text-rose-500 hover:bg-rose-50 transition-all text-left">
              ...
              <p className="text-[10px] text-slate-400 leading-relaxed">
                This is permanent and cannot be undone. All milestones associated with this project will also be removed.
              </p>
              <div className="flex gap-2 pt-1">
                <button type="button" onClick={() => setConfirming(false)} className="flex-1 py-2 rounded-xl bg-slate-100 text-slate-600 text-xs font-bold hover:bg-slate-200 transition-all">
                  Cancel
                </button>
                <button type="button" onClick={handleDelete} disabled={busy} className="flex-1 py-2 rounded-xl bg-rose-500 text-white text-xs font-bold hover:bg-rose-600 active:scale-95 transition-all disabled:opacity-60">
...
const ProjectHeader = () => {
    ...
    return (<div className="bg-white border-b border-slate-100 sticky top-0 z-40">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
          <div>
            ...
            <h2 className="text-4xl font-extrabold text-primary tracking-tighter leading-none font-headline flex items-center gap-4 flex-wrap">
              ...
              <button id="copy-project-id-btn" type="button" onClick={handleCopyId} title={copied ? 'Copied!' : 'Copy project ID'} className={`
            ...
            : 'bg-emerald-50 text-emerald-700 border-emerald-100/50 hover:bg-emerald-100 hover:border-emerald-200'}
                `}>
                {!copied && (<span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"/>)}
                <span className="font-mono tracking-wider">
                  {copied ? 'COPIED!' : `ID: ${id}`}
                </span>
                <span className={`material-symbols-outlined text-[14px] transition-all duration-200 ${copied ? 'opacity-100 scale-100' : 'opacity-0 scale-75 group-hover:opacity-100 group-hover:scale-100'}`}>
            ...
            return (<span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border transition-colors duration-300 ${sc.bg} ${sc.text} border-current/10`}>
                    <span className={`w-1.5 h-1.5 rounded-full ${sc.dot} ${selectedProject.status === ProjectStatus.IN_PROGRESS ? 'animate-pulse' : ''}`}/>
