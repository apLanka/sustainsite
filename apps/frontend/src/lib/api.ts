...
export const projectApi = {
  ...
  addMilestone: async (
    ...
    const response = await api.post<SingleResponse<Milestone>>(
...
export const resourcesApi = {
  ...
  updateMaterialStatus: async (
    id: string,
    status: string
  ): Promise<SingleResponse<MaterialAsset>> => {
    const response = await api.put<SingleResponse<MaterialAsset>>(
      `/resources/materials/${id}/status`,
      { status }
    );
    return response.data;
  },
  ...
  ): Promise<SingleResponse<EquipmentAsset>> => {
  ...
  updateEquipment: async (
    ...
    const response = await api.put<SingleResponse<EquipmentAsset>>(
      `/resources/equipment/${id}`,
      data
    );
    return response.data;
  },
  ...
  assignEquipment: async (
    id: string,
    data: AssignEquipmentPayload
  ): Promise<SingleResponse<EquipmentAsset>> => {
    const response = await api.post<SingleResponse<EquipmentAsset>>(
      `/resources/equipment/${id}/assign`,
      data
    );
    return response.data;
  },
...
export const sustainabilityApi = {
  ...
  getProjectMetrics: async (
    projectId: string,
    limit = 10
  ): Promise<PaginatedResponse<SustainabilityMetric>> => {
    const response = await api.get<PaginatedResponse<SustainabilityMetric>>(
      `/sustainability/projects/${projectId}/metrics`,
      { params: { limit } }
    );
    return response.data;
  },
  ...
  compareWithIndustry: async (
    projectId: string
  ): Promise<SingleResponse<IndustryComparisonResponse>> => {
  ...
  calculateImpact: async (data: {
    ...
    const response = await api.post<SingleResponse<ImpactCalculationResponse>>(
      '/sustainability/calculate-impact',
      data
    );
    return response.data;
  },
...
export const documentApi = {
  ...
  ): Promise<SingleResponse<ProjectDocument>> => {
  ...
  reject: async (id: string, rejectionReason: string): Promise<SingleResponse<ProjectDocument>> => {
    const response = await api.put<SingleResponse<ProjectDocument>>(`/documents/${id}/reject`, {
      rejectionReason,
    });
    return response.data;
  },
  createVersion: async (id: string, file: File): Promise<SingleResponse<ProjectDocument>> => {
    ...
    const response = await api.post<SingleResponse<ProjectDocument>>(
...
export const complianceApi = {
  createChecklist: async (
    ...
    const res = await api.post<SingleResponse<ComplianceChecklist>>(
  ...
  getChecklists: async (
    filters: Partial<ChecklistFilters>
  ): Promise<CompliancePaginatedResponse<ComplianceChecklist>> => {
    const params: Record<string, string> = {};
    if (filters.projectId) params.projectId = filters.projectId;
    if (filters.category) params.category = filters.category;
    if (filters.page) params.page = String(filters.page);
    if (filters.limit) params.limit = String(filters.limit);
    const res = await api.get<CompliancePaginatedResponse<ComplianceChecklist>>(
      '/compliance/checklists',
      { params }
    );
    return res.data;
  ...
  ): Promise<SingleResponse<SafetyInspection>> => {
    const res = await api.post<SingleResponse<SafetyInspection>>(
  ...
  getInspections: async (
  ...
  ): Promise<CompliancePaginatedResponse<SafetyInspection>> => {
    const params: Record<string, string> = {};
    if (filters.projectId) params.projectId = filters.projectId;
    ...
    if (filters.page) params.page = String(filters.page);
    if (filters.limit) params.limit = String(filters.limit);
    const res = await api.get<CompliancePaginatedResponse<SafetyInspection>>(
      '/compliance/inspections',
      { params }
    );
    return res.data;
  ...
  updateInspection: async (
    ...
    const res = await api.put<SingleResponse<SafetyInspection>>(
  ...
  updateChecklistItem: async (
    ...
    const res = await api.put<SingleResponse<ComplianceChecklist>>(
      `/compliance/checklists/${checklistId}/items/${itemId}`,
      payload
    );
    return res.data;
  },
...
export const dashboardApi = {
  ...
  getPendingApprovals: async (): Promise<number> => {
    const res = await api.get<DocumentPaginatedResponse>('/documents', {
      params: { status: 'Under Review', limit: 1 },
    });
    return res.data.pagination.total;
  },
  getHighRiskCount: async (): Promise<number> => {
    const [high, critical] = await Promise.all([
      api.get<CompliancePaginatedResponse<SafetyInspection>>('/compliance/inspections', {
        params: { riskLevel: 'High', isResolved: 'false', limit: 1 },
      }),
      api.get<CompliancePaginatedResponse<SafetyInspection>>('/compliance/inspections', {
        params: { riskLevel: 'Critical', isResolved: 'false', limit: 1 },
      }),
    ]);
