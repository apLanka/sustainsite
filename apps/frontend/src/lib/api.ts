...
import type { CreateMilestonePayload, CreateProjectPayload, Milestone, Project, ProjectFilters, UpdateMilestonePayload, UpdateProjectPayload, } from '@/types/project';
import type { DocumentFilters, DocumentPagination, ProjectDocument, UpdateDocumentPayload, UploadDocumentPayload, } from '@/types/document';
import type { ChecklistFilters, ComplianceChecklist, CompliancePagination, CreateChecklistPayload, CreateInspectionPayload, InspectionFilters, ProjectComplianceScore, SafetyInspection, UpdateChecklistItemPayload, UpdateChecklistPayload, UpdateInspectionPayload, } from '@/types/compliance';
import type { AssignEquipmentPayload, CreateEquipmentPayload, CreateMaterialPayload, CreateSupplierPayload, EquipmentAsset, FinancialSummary, MaintenancePayload, MaterialAsset, ResourceSummary, Supplier, UpdateEquipmentPayload, UpdateMaterialPayload, UpdateSupplierPayload, } from '@/types/resources';
import type { ImpactCalculationResponse, IndustryComparisonResponse, SustainabilityMetric, SustainabilityScore, SustainabilityTrend, SustainabilityTrendsResponse, } from '@/types/sustainability';
...
export const projectApi = {
    ...
    updateMilestone: async (projectId: string, milestoneId: string, data: UpdateMilestonePayload): Promise<SingleResponse<Milestone>> => {
        const response = await api.put<SingleResponse<Milestone>>(`/projects/${projectId}/milestones/${milestoneId}`, data);
        return response.data;
    },
    getTimeline: async (id: string): Promise<SingleResponse<{
...
export const resourcesApi = {
    getMaterials: async (projectId: string | undefined, page = 1, limit = 10): Promise<PaginatedResponse<MaterialAsset>> => {
        const params = new URLSearchParams({
            page: String(page),
            limit: String(limit),
        });
        ...
        const response = await api.get<PaginatedResponse<MaterialAsset>>(`/resources/materials?${params.toString()}`);
        return response.data;
    },
    ...
    getEquipment: async (projectId: string, status?: string, page = 1, limit = 10): Promise<PaginatedResponse<EquipmentAsset>> => {
        const params = new URLSearchParams({
            projectId,
            page: String(page),
            limit: String(limit),
        ...
        const response = await api.get<PaginatedResponse<EquipmentAsset>>(`/resources/equipment?${params.toString()}`);
        return response.data;
    },
    getAvailableEquipment: async (page = 1, limit = 10): Promise<PaginatedResponse<EquipmentAsset>> => {
        const params = new URLSearchParams({
            page: String(page),
            limit: String(limit),
        });
        const response = await api.get<PaginatedResponse<EquipmentAsset>>(`/resources/equipment/list/available?${params.toString()}`);
        return response.data;
    },
    getSuppliers: async (isActive = true, page = 1, limit = 10): Promise<PaginatedResponse<Supplier>> => {
        const params = new URLSearchParams({
            isActive: String(isActive),
            page: String(page),
            limit: String(limit),
        });
        const response = await api.get<PaginatedResponse<Supplier>>(`/resources/suppliers?${params.toString()}`);
        return response.data;
    },
    ...
    getCostSummary: async (projectId: string): Promise<SingleResponse<ResourceSummary>> => {
        const response = await api.get<SingleResponse<ResourceSummary>>(`/resources/materials/${projectId}/cost-summary`);
        return response.data;
    },
};
export const sustainabilityApi = {
    ...
    getProjectScore: async (projectId: string): Promise<SingleResponse<SustainabilityScore>> => {
        const response = await api.get<SingleResponse<SustainabilityScore>>(`/sustainability/projects/${projectId}/score`);
        return response.data;
    },
    getProjectTrends: async (projectId: string): Promise<SingleResponse<SustainabilityTrend[]>> => {
        const response = await api.get<SingleResponse<SustainabilityTrend[]>>(`/sustainability/projects/${projectId}/trends`);
        return response.data;
    },
    getLatestMetric: async (projectId: string): Promise<SingleResponse<SustainabilityMetric>> => {
        const response = await api.get<SingleResponse<SustainabilityMetric>>(`/sustainability/projects/${projectId}/metrics/latest`);
        return response.data;
    },
    getProjectMetrics: async (projectId: string, limit = 10): Promise<PaginatedResponse<SustainabilityMetric>> => {
        const response = await api.get<PaginatedResponse<SustainabilityMetric>>(`/sustainability/projects/${projectId}/metrics`, { params: { limit } });
        return response.data;
    },
    getProjectTrendsDetailed: async (projectId: string, period = '6months', interval = 'monthly'): Promise<SingleResponse<SustainabilityTrendsResponse>> => {
        const response = await api.get<SingleResponse<SustainabilityTrendsResponse>>(`/sustainability/projects/${projectId}/trends`, { params: { period, interval } });
        return response.data;
    },
    compareWithIndustry: async (projectId: string): Promise<SingleResponse<IndustryComparisonResponse>> => {
        const response = await api.get<SingleResponse<IndustryComparisonResponse>>(`/sustainability/projects/${projectId}/compare`);
        return response.data;
    },
    calculateImpact: async (data: {
        ...
        const response = await api.post<SingleResponse<ImpactCalculationResponse>>('/sustainability/calculate-impact', data);
        return response.data;
    },
};
export const documentApi = {
    ...
    getDownloadUrl: (id: string): string => `${API_BASE_URL}/documents/${id}/download`,
};
...
export const complianceApi = {
    ...
    getInspections: async (filters: Partial<InspectionFilters>): Promise<CompliancePaginatedResponse<SafetyInspection>> => {
        ...
        if (filters.actionStatus)
            params.actionStatus = filters.actionStatus;
        if (filters.inspectionType)
            params.inspectionType = filters.inspectionType;
    ...
    updateChecklistItem: async (checklistId: string, itemId: string, payload: UpdateChecklistItemPayload): Promise<SingleResponse<ComplianceChecklist>> => {
        const res = await api.put<SingleResponse<ComplianceChecklist>>(`/compliance/checklists/${checklistId}/items/${itemId}`, payload);
        return res.data;
    },
    getProjectScore: async (projectId: string): Promise<SingleResponse<ProjectComplianceScore>> => {
        const res = await api.get<SingleResponse<ProjectComplianceScore>>(`/compliance/score/${projectId}`);
        return res.data;
    },
};
export const safetyApi = {
    ...
    getByProject: async (projectId: string, params?: Partial<InspectionFilters>): Promise<{
        success: boolean;
        data: SafetyInspection[];
        count?: number;
        pagination?: CompliancePagination;
    }> => {
    ...
    getHighRisk: async (projectId: string): Promise<{
        success: boolean;
        data: SafetyInspection[];
        count: number;
    }> => {
