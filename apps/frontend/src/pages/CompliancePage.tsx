...
const RiskBadge = ({ level }: { level: RiskLevel }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${c.bg} ${c.text}`}
    >
...
const ActionBadge = ({ status }: { status: ActionStatus }) => {
  ...
  return (
    <span
      className={`inline-flex items-center px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${c.bg} ${c.text}`}
    >
      {status}
    </span>
  );
...
export default function CompliancePage() {
  ...
  useEffect(() => {
    ...
    projectId,
    inspectionFilters.page,
    inspectionFilters.riskLevel,
    inspectionFilters.actionStatus,
    setInspections,
    setInspectionLoading,
  ]);
  ...
  const handleToggleItem = async (itemId: string) => {
    ...
    try {
      const res = await complianceApi.updateChecklist(selectedChecklist._id, {
        items: updatedItems,
      });
  ...
  const handleAddChecklistItem = () => {
    ...
    setChecklistItems((prev) => [
      ...prev,
      {
        itemId: `item-${Date.now()}-${prev.length}`,
              ...
              {upcomingDueDates.length === 0 ? (
                ...
                <ul className="space-y-4">
                  {upcomingDueDates.map((c) => (
                    <li
                      key={c._id}
                      className="flex gap-4 items-start border-l border-emerald-800/50 pl-4 py-1 relative"
                    >
                      ...
                      <div className="text-xs font-bold text-emerald-50 mt-[-2px] truncate">
                        {c.checklistName}
                      </div>
                    </li>
                  ))}
            ...
            {isChecklistLoading ? (
              ...
              <div className="bg-surface-container-lowest rounded-3xl p-10 border border-slate-100/50 shadow-sm flex flex-col items-center justify-center py-24 text-center">
                ...
                <p className="text-xs text-slate-300 mt-1">
                  Create a checklist to start tracking compliance
                </p>
              </div>
            ) : (
                        ...
                        <div className="flex justify-between items-start gap-4">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                              <h5
                                className={`text-sm font-bold leading-tight ${item.isCompleted ? 'text-emerald-700 line-through decoration-emerald-300' : 'text-primary'}`}
                              >
                                {item.itemName}
                              </h5>
                            </div>
                            ...
                            <div className="flex items-center gap-3 mt-2 flex-wrap">
                              ...
                              {item.attachedDocuments?.length > 0 && (
                                <div className="flex items-center gap-1 text-[10px] font-bold text-secondary uppercase tracking-tight bg-secondary/10 px-2 py-0.5 rounded border border-secondary/20">
                                  ...
                                  {item.attachedDocuments.length} doc
                                  {item.attachedDocuments.length !== 1 ? 's' : ''}
                                </div>
                              )}
                              {item.notes && (
            ...
            <div className="flex justify-between items-center mb-8">
              ...
              <div className="flex items-center gap-2">
                <select
                  ...
                  onChange={(e) =>
                    setInspectionFilters({ riskLevel: e.target.value as RiskLevel | '', page: 1 })
                  }
                >
                ...
                <select
                  ...
                  onChange={(e) =>
                    setInspectionFilters({
                      actionStatus: e.target.value as ActionStatus | '',
                      page: 1,
                    })
                  }
                >
                  <option value="">All Statuses</option>
                  {ALL_ACT_STATUSES.map((s) => (
                      ...
                      <td className="px-4 py-4">
                        {canInspect ? (
                          <select
                            ...
                            onChange={(e) =>
                              handleUpdateActionStatus(insp._id, e.target.value as ActionStatus)
                            }
                          >
                            {ALL_ACT_STATUSES.map((s) => (
                              <option key={s} value={s}>
            ...
            <div className="p-10 space-y-5 max-h-[72vh] overflow-y-auto">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setChecklistForm((f) => ({ ...f, checklistName: e.target.value }))
                  }
                />
              </div>
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setChecklistForm((f) => ({
                      ...f,
                      category: (e.target.value as ComplianceCategory) || undefined,
                    }))
                  }
                >
                ...
                {checklistItems.length > 0 && (
                  <ul className="space-y-2">
                    {checklistItems.map((item, i) => (
                      ...
                      >
                        <span className="w-5 h-5 mt-0.5 rounded-full bg-slate-200 text-slate-500 text-[10px] font-black flex items-center justify-center shrink-0">
                          {i + 1}
                        </span>
                        <div className="flex-1 min-w-0">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        setNewItemDraft((d) => ({ ...d, description: e.target.value }))
                      }
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                          e.preventDefault();
      ...
      {detailInspection && inspectionDetail && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <div className="bg-white rounded-[40px] w-full max-w-xl relative overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-300">
            <div
              className={`p-8 text-white ${inspectionDetail.riskLevel === RiskLevel.CRITICAL || inspectionDetail.riskLevel === RiskLevel.HIGH ? 'bg-rose-700' : 'bg-primary'}`}
            >
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-white/50 text-xs font-bold uppercase tracking-widest mt-2">
                    {fmt(inspectionDetail.inspectionDate)} ·{' '}
                    {inspectionDetail.inspectionType ?? 'General'}
                  </p>
                </div>
            ...
            <div className="p-10 space-y-5 max-h-[65vh] overflow-y-auto">
              <div>
                ...
                <p className="text-sm font-bold text-primary">
                  {inspectionDetail.inspector?.fullName ?? '—'}
                </p>
              </div>
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">
                ...
                <div>
                  ...
                  {inspectionDetail.actionDeadline && (
                    <p className="text-[10px] font-bold text-rose-500 mt-1">
                      Deadline: {fmt(inspectionDetail.actionDeadline)}
                    </p>
                  )}
                </div>
              )}
                ...
                <div>
                  ...
                  <ul className="space-y-2">
                    {inspectionDetail.issuesIdentified.map((issue, i) => (
                      <li
                        key={i}
                        className="flex items-start gap-3 bg-slate-50 p-3 rounded-xl text-xs font-medium text-slate-600"
                      >
                        <span
                          className={`px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wider shrink-0 ${
                            issue.severity === IssueSeverity.MAJOR
                              ...
                              : issue.severity === IssueSeverity.MODERATE
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <div className="bg-white rounded-[40px] w-full max-w-sm relative overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-300 p-10">
            ...
            <p className="text-xs text-slate-400 leading-relaxed mb-8">
              This will permanently remove the checklist and all its items.
            </p>
      ...
      {deletingInspectionId && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <div className="bg-white rounded-[40px] w-full max-w-sm relative overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-300 p-10">
            ...
            <p className="text-xs text-slate-400 leading-relaxed mb-8">
              This inspection record will be permanently removed.
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => setDeletingInspectionId(null)}
