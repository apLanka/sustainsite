...
import { ComplianceCategory, InspectionType, RiskLevel, ActionStatus, IssueSeverity, } from '@/types/compliance';
import type { ComplianceItem, IssueIdentified, CreateChecklistPayload, CreateInspectionPayload, } from '@/types/compliance';
import { UserRole } from '@/types/auth';
const fmt = (iso?: string) => iso ? new Date(iso).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '—';
const fmtShort = (iso?: string) => iso ? new Date(iso).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : '—';
...
const RiskBadge = ({ level }: {
    ...
    return (<span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${c.bg} ${c.text}`}>
...
const ActionBadge = ({ status }: {
    ...
    return (<span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${c.bg} ${c.text}`}>
      {status}
    </span>);
};
export default function CompliancePage() {
    ...
    const { checklists, selectedChecklist, checklistPagination, isChecklistLoading, inspections, inspectionPagination, isInspectionLoading, inspectionFilters, setChecklists, setSelectedChecklist, appendChecklist, updateChecklistInStore, removeChecklist, setInspections, appendInspection, updateInspectionInStore, removeInspection, setChecklistLoading, setInspectionLoading, setInspectionFilters, resetComplianceFilters, } = useComplianceStore();
    ...
    const inspectionDetail = useMemo(() => inspections.find((i) => i._id === detailInspection) ?? null, [inspections, detailInspection]);
    ...
    const handleToggleItem = async (itemId: string) => {
        if (!selectedChecklist || isSavingItems)
            return;
        const snapshot = selectedChecklist.items;
        const updatedItems: ComplianceItem[] = snapshot.map((item) => item.itemId === itemId ? { ...item, isCompleted: !item.isCompleted } : item);
    ...
    return (<DashboardLayout>
        ...
        <header className="py-10 flex flex-col md:flex-row justify-between items-end md:items-center gap-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative group">
              <select className="input-standard bg-surface-container-lowest h-11 py-0 pr-10 appearance-none cursor-pointer text-xs font-bold" value={selectedChecklist?._id ?? ''} onChange={(e) => handleSelectChecklist(e.target.value)} disabled={isChecklistLoading || checklists.length === 0}>
                {checklists.length === 0
            ? <option value="">No checklists yet</option>
            : checklists.map((c) => (<option key={c._id} value={c._id}>{c.checklistName}</option>))}
              </select>
            ...
            <button onClick={() => { setChecklistError(null); setShowChecklistModal(true); }} className="px-4 py-2.5 bg-slate-50 border border-slate-200 text-primary font-bold text-xs rounded-xl flex items-center gap-2 hover:bg-slate-100 transition-all cursor-pointer">
              <span className="material-symbols-outlined text-base">playlist_add</span>
              New Checklist
            </button>

            {canInspect && (<button onClick={() => { setInspError(null); setShowInspectionModal(true); }} className="px-5 py-2.5 bg-primary text-white font-bold text-xs rounded-xl flex items-center gap-2 shadow-lg shadow-primary/10 hover:brightness-110 active:scale-95 transition-all cursor-pointer">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-1 space-y-8">
            ...
            <div className="bg-surface-container-lowest p-10 rounded-3xl border border-slate-100/50 shadow-sm text-center relative overflow-hidden">
              ...
              <div className="relative inline-flex items-center justify-center">
                <svg className="w-48 h-48 -rotate-90" viewBox="0 0 192 192">
                  ...
                  <circle cx="96" cy="96" r={CIRC_R} stroke="currentColor" strokeWidth="12" fill="transparent" strokeDasharray={CIRC_LEN} strokeDashoffset={CIRC_LEN * (1 - avgScore / 100)} strokeLinecap="round" className="text-secondary transition-all duration-1000"/>
                </svg>
            ...
            <div className="bg-emerald-950 p-8 rounded-3xl shadow-xl shadow-emerald-950/20 text-white relative overflow-hidden">
              ...
              {upcomingDueDates.length === 0 ? (<p className="text-xs text-emerald-400/50 font-medium">No upcoming due dates</p>) : (<ul className="space-y-4">
                  {upcomingDueDates.map((c) => (<li key={c._id} className="flex gap-4 items-start border-l border-emerald-800/50 pl-4 py-1 relative">
          ...
          <div className="lg:col-span-2">
            {isChecklistLoading ? (<div className="bg-surface-container-lowest rounded-3xl p-10 border border-slate-100/50 animate-pulse space-y-4">
                {Array.from({ length: 4 }).map((_, i) => (<div key={i} className="h-20 bg-slate-100 rounded-2xl"/>))}
              </div>) : !selectedChecklist ? (<div className="bg-surface-container-lowest rounded-3xl p-10 border border-slate-100/50 shadow-sm flex flex-col items-center justify-center py-24 text-center">
              ...
              </div>) : (<div className="bg-surface-container-lowest rounded-3xl p-10 border border-slate-100/50 shadow-sm space-y-6">
                ...
                <div className="flex justify-between items-start pb-6 border-b border-slate-50">
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-1.5">
                      {selectedChecklist.category && (<span className="px-2.5 py-1 bg-slate-100 rounded-lg text-[9px] font-black text-slate-500 uppercase tracking-widest">
                          {selectedChecklist.category}
                        </span>)}
                      <span className="text-[10px] font-bold text-secondary uppercase tracking-widest">
                        {selectedChecklist.complianceScore}% complete
                      </span>
                      {selectedChecklist.dueDate && (<span className="text-[10px] font-medium text-slate-400">
                          Due {fmt(selectedChecklist.dueDate)}
                        </span>)}
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    {isSavingItems && (<span className="w-4 h-4 border-2 border-slate-200 border-t-secondary rounded-full animate-spin"/>)}
                    {isAdmin && (<button onClick={() => setDeletingChecklistId(selectedChecklist._id)} className="p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-all" title="Delete checklist">
                ...
                <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-secondary to-emerald-400 rounded-full transition-all duration-700" style={{ width: `${selectedChecklist.complianceScore}%` }}/>
                </div>

                
                {selectedChecklist.items.length === 0 ? (<p className="text-xs text-slate-400 font-medium text-center py-8">
                    No items in this checklist yet.
                  </p>) : (<div className="space-y-4">
                    {selectedChecklist.items.map((item) => (<div key={item.itemId} className={`p-6 border rounded-2xl group transition-all ${item.isCompleted
                        ...
                        <div className="flex justify-between items-start gap-4">
                          <div className="flex-1 min-w-0">
                            ...
                            {item.description && (<p className="text-[11px] text-slate-400 font-medium mt-1 leading-relaxed">
                                {item.description}
                              </p>)}
                            <div className="flex items-center gap-3 mt-2 flex-wrap">
                              {item.completedBy && (<div className="flex items-center gap-1 text-[10px] font-bold text-emerald-600 uppercase tracking-tight bg-emerald-50 px-2 py-0.5 rounded border border-emerald-100">
                                  <span className="material-symbols-outlined text-[12px]">person</span>
                                  {item.completedBy.fullName}
                                </div>)}
                              {item.attachedDocuments?.length > 0 && (<div className="flex items-center gap-1 text-[10px] font-bold text-secondary uppercase tracking-tight bg-secondary/10 px-2 py-0.5 rounded border border-secondary/20">
                                  <span className="material-symbols-outlined text-[12px]">attach_file</span>
                                  {item.attachedDocuments.length} doc{item.attachedDocuments.length !== 1 ? 's' : ''}
                                </div>)}
                              {item.notes && (<p className="text-[10px] text-slate-400 font-medium">{item.notes}</p>)}
                            </div>
                          </div>
                          <button onClick={() => handleToggleItem(item.itemId)} disabled={isSavingItems} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shrink-0 disabled:opacity-60 ${item.isCompleted
        ...
        <div className="mt-10 pb-20">
          <div className="bg-surface-container-lowest rounded-3xl p-10 border border-slate-100/50 shadow-sm overflow-hidden">
            <div className="flex justify-between items-center mb-8">
              ...
              <div className="flex items-center gap-2">
                <select className="input-standard h-9 py-0 text-xs font-bold cursor-pointer" value={inspectionFilters.riskLevel ?? ''} onChange={(e) => setInspectionFilters({ riskLevel: e.target.value as RiskLevel | '', page: 1 })}>
                  <option value="">All Risks</option>
                  {ALL_RISK_LEVELS.map((r) => <option key={r} value={r}>{r}</option>)}
                </select>
                <select className="input-standard h-9 py-0 text-xs font-bold cursor-pointer" value={inspectionFilters.actionStatus ?? ''} onChange={(e) => setInspectionFilters({ actionStatus: e.target.value as ActionStatus | '', page: 1 })}>
            ...
            {isInspectionLoading ? (<div className="animate-pulse space-y-3">
                {Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-14 bg-slate-100 rounded-xl"/>)}
              </div>) : inspections.length === 0 ? (<div className="text-center py-16">
                ...
                <tbody className="divide-y divide-slate-50">
                  {inspections.map((insp) => (<tr key={insp._id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-4 py-4">
                        {canInspect ? (<select className="input-standard h-8 py-0 px-2 text-[10px] font-black cursor-pointer" value={insp.actionStatus} onChange={(e) => handleUpdateActionStatus(insp._id, e.target.value as ActionStatus)}>
                            {ALL_ACT_STATUSES.map((s) => <option key={s} value={s}>{s}</option>)}
                          </select>) : (<ActionBadge status={insp.actionStatus}/>)}
                      </td>
                      <td className="px-4 py-4 text-right">
                        <div className="flex justify-end gap-1">
                          <button onClick={() => setDetailInspection(insp._id)} className="p-1.5 text-slate-400 hover:text-secondary hover:bg-slate-100 rounded-lg transition-all" title="View details">
                            <span className="material-symbols-outlined text-lg">open_in_new</span>
                          </button>
                          {isAdmin && (<button onClick={() => setDeletingInspectionId(insp._id)} className="p-1.5 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-all" title="Delete">
                              <span className="material-symbols-outlined text-lg">delete_outline</span>
                            </button>)}
                        </div>
                      </td>
            ...
            {inspectionPagination && inspectionPagination.pages > 1 && (<div className="flex justify-center items-center gap-3 pt-6 border-t border-slate-50 mt-4">
                <button className="px-4 py-2 text-xs font-bold text-slate-500 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed" disabled={inspectionFilters.page <= 1} onClick={() => setInspectionFilters({ page: inspectionFilters.page - 1 })}>
                  Previous
                </button>
                <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                  {inspectionFilters.page} / {inspectionPagination.pages}
                </span>
                <button className="px-4 py-2 text-xs font-bold text-slate-500 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed" disabled={inspectionFilters.page >= inspectionPagination.pages} onClick={() => setInspectionFilters({ page: inspectionFilters.page + 1 })}>
                  Next
                </button>
              </div>)}
          </div>
      ...
      {showChecklistModal && (<div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <div className="bg-white rounded-[40px] w-full max-w-lg relative overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-300">
            ...
            <div className="p-10 space-y-5 max-h-[72vh] overflow-y-auto">
              {checklistError && (<div className="p-3 bg-rose-50 border border-rose-200 rounded-xl text-sm text-rose-600 font-medium">{checklistError}</div>)}
              <div className="space-y-1.5">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Checklist Name *</label>
                <input type="text" placeholder="e.g., Environmental Compliance (EIA)" className="input-standard w-full h-12" value={checklistForm.checklistName} onChange={(e) => setChecklistForm((f) => ({ ...f, checklistName: e.target.value }))}/>
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Category</label>
                <select className="input-standard w-full h-12 py-0 cursor-pointer" value={checklistForm.category ?? ''} onChange={(e) => setChecklistForm((f) => ({ ...f, category: e.target.value as ComplianceCategory || undefined }))}>
              ...
              <div className="space-y-1.5">
                ...
                <input type="date" className="input-standard w-full h-12 cursor-pointer" value={checklistForm.dueDate ?? ''} onChange={(e) => setChecklistForm((f) => ({ ...f, dueDate: e.target.value }))}/>
              </div>
              ...
              <div className="space-y-3 pt-2">
                ...
                {checklistItems.length > 0 && (<ul className="space-y-2">
                    {checklistItems.map((item, i) => (<li key={item.itemId} className="flex items-start gap-3 bg-slate-50 border border-slate-100 rounded-xl px-4 py-3">
                        ...
                        <button type="button" onClick={() => setChecklistItems((prev) => prev.filter((_, j) => j !== i))} className="text-slate-300 hover:text-rose-400 transition-colors shrink-0">
                ...
                <div className="space-y-2">
                  <input type="text" className="input-standard w-full h-10 text-sm" placeholder="Item name (e.g. Fire extinguisher inspection)" value={newItemDraft.itemName} onChange={(e) => setNewItemDraft((d) => ({ ...d, itemName: e.target.value }))} onKeyDown={(e) => { if (e.key === 'Enter') {
            e.preventDefault();
            handleAddChecklistItem();
        } }}/>
                  <div className="flex gap-2">
                    <input type="text" className="input-standard flex-1 h-10 text-sm" placeholder="Description (optional)" value={newItemDraft.description} onChange={(e) => setNewItemDraft((d) => ({ ...d, description: e.target.value }))} onKeyDown={(e) => { if (e.key === 'Enter') {
            e.preventDefault();
            handleAddChecklistItem();
        } }}/>
                    <button type="button" onClick={handleAddChecklistItem} disabled={!newItemDraft.itemName.trim()} className="px-4 h-10 bg-slate-100 text-primary rounded-xl text-xs font-bold hover:bg-slate-200 transition-all disabled:opacity-40 disabled:cursor-not-allowed">
                      Add
                    </button>
                  </div>
                </div>
              ...
              <div className="flex gap-4 pt-2">
                ...
                <button onClick={handleCreateChecklist} disabled={isCreatingChecklist} className="flex-[2] py-4 bg-primary text-white font-black rounded-2xl shadow-xl shadow-primary/20 hover:brightness-110 active:scale-95 transition-all cursor-pointer disabled:opacity-60">
      ...
      {showInspectionModal && (<div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <div className="bg-white rounded-[40px] w-full max-w-2xl relative overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-300">
            ...
            <div className="p-10 space-y-5 max-h-[72vh] overflow-y-auto">
              {inspError && (<div className="p-3 bg-rose-50 border border-rose-200 rounded-xl text-sm text-rose-600 font-medium">{inspError}</div>)}

              <div className="grid grid-cols-2 gap-5">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Inspection Date *</label>
                  <input type="date" className="input-standard w-full h-12 cursor-pointer" value={inspForm.inspectionDate} onChange={(e) => setInspForm((f) => ({ ...f, inspectionDate: e.target.value }))}/>
                </div>
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Type</label>
                  <select className="input-standard w-full h-12 py-0 cursor-pointer" value={inspForm.inspectionType ?? ''} onChange={(e) => setInspForm((f) => ({ ...f, inspectionType: e.target.value as InspectionType || undefined }))}>
              ...
              <div className="space-y-1.5">
                ...
                <div className="grid grid-cols-4 gap-2">
                  {ALL_RISK_LEVELS.map((r) => (<button key={r} type="button" onClick={() => setInspForm((f) => ({ ...f, riskLevel: r }))} className={`py-2.5 rounded-xl text-[10px] font-black uppercase tracking-wider border transition-all ${inspForm.riskLevel === r
              ...
              <div className="space-y-1.5">
                ...
                <textarea rows={3} className="input-standard w-full resize-none" placeholder="Describe the inspection findings..." value={inspForm.findings} onChange={(e) => setInspForm((f) => ({ ...f, findings: e.target.value }))}/>
              </div>

              <div className="space-y-1.5">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Inspector Notes</label>
                <textarea rows={2} className="input-standard w-full resize-none" placeholder="Additional notes from the inspector..." value={inspForm.inspectorNotes ?? ''} onChange={(e) => setInspForm((f) => ({ ...f, inspectorNotes: e.target.value }))}/>
              </div>

              <div className="grid grid-cols-2 gap-5">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Action Required</label>
                  <input type="text" className="input-standard w-full h-12" placeholder="Describe required actions..." value={inspForm.actionRequired ?? ''} onChange={(e) => setInspForm((f) => ({ ...f, actionRequired: e.target.value }))}/>
                </div>
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Action Deadline</label>
                  <input type="date" className="input-standard w-full h-12 cursor-pointer" value={inspForm.actionDeadline ?? ''} onChange={(e) => setInspForm((f) => ({ ...f, actionDeadline: e.target.value }))}/>
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex gap-2">
                  <input type="text" className="input-standard flex-1 h-10" placeholder="Add a recommended action…" value={recAction} onChange={(e) => setRecAction(e.target.value)} onKeyDown={(e) => {
                if (e.key === 'Enter' && recAction.trim()) {
                    setInspForm((f) => ({ ...f, recommendedActions: [...(f.recommendedActions ?? []), recAction.trim()] }));
                    setRecAction('');
                }
            }}/>
                  <button type="button" onClick={() => { if (recAction.trim()) {
            setInspForm((f) => ({ ...f, recommendedActions: [...(f.recommendedActions ?? []), recAction.trim()] }));
            setRecAction('');
        } }} className="px-3 h-10 bg-slate-100 text-primary rounded-xl text-xs font-bold hover:bg-slate-200 transition-all">Add</button>
                </div>
                {(inspForm.recommendedActions ?? []).length > 0 && (<ul className="space-y-1 mt-2">
                    {(inspForm.recommendedActions ?? []).map((a, i) => (<li key={i} className="flex items-center justify-between bg-slate-50 px-3 py-2 rounded-lg text-xs font-medium text-slate-600">
                        <span>{a}</span>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-4 gap-2">
                  <input type="text" className="input-standard h-10 col-span-2" placeholder="Issue description" value={newIssue.issue} onChange={(e) => setNewIssue((n) => ({ ...n, issue: e.target.value }))}/>
                  <input type="text" className="input-standard h-10" placeholder="Location (optional)" value={newIssue.location} onChange={(e) => setNewIssue((n) => ({ ...n, location: e.target.value }))}/>
                  <select className="input-standard h-10 py-0 cursor-pointer" value={newIssue.severity} onChange={(e) => setNewIssue((n) => ({ ...n, severity: e.target.value as IssueSeverity }))}>
                    {ALL_SEVERITIES.map((s) => <option key={s} value={s}>{s}</option>)}
                  </select>
                </div>
                <button type="button" onClick={() => { if (newIssue.issue.trim()) {
            setInspIssues((prev) => [...prev, { ...newIssue }]);
            setNewIssue({ issue: '', severity: IssueSeverity.MINOR, location: '' });
        } }} className="px-3 h-10 bg-slate-100 text-primary rounded-xl text-xs font-bold hover:bg-slate-200 transition-all w-full">Add Issue</button>
                {inspIssues.length > 0 && (<ul className="space-y-1 mt-1">
                    {inspIssues.map((issue, i) => (<li key={i} className="flex items-center justify-between bg-slate-50 px-3 py-2 rounded-lg text-xs font-medium text-slate-600">
              ...
              <div className="flex gap-4 pt-2">
                ...
                <button onClick={handleCreateInspection} disabled={isCreatingInsp} className="flex-[2] py-4 bg-primary text-white font-black rounded-2xl shadow-xl shadow-primary/20 hover:brightness-110 active:scale-95 transition-all cursor-pointer disabled:opacity-60">
      ...
      {detailInspection && inspectionDetail && (<div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <div className="bg-white rounded-[40px] w-full max-w-xl relative overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-300">
            ...
            <div className="p-10 space-y-5 max-h-[65vh] overflow-y-auto">
              ...
              {inspectionDetail.inspectorNotes && (<div>
              ...
              {inspectionDetail.actionRequired && (<div>
                  ...
                  {inspectionDetail.actionDeadline && (<p className="text-[10px] font-bold text-rose-500 mt-1">Deadline: {fmt(inspectionDetail.actionDeadline)}</p>)}
                </div>)}
              {inspectionDetail.issuesIdentified.length > 0 && (<div>
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Issues Identified</p>
                  <ul className="space-y-2">
                    {inspectionDetail.issuesIdentified.map((issue, i) => (<li key={i} className="flex items-start gap-3 bg-slate-50 p-3 rounded-xl text-xs font-medium text-slate-600">
                        <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wider shrink-0 ${issue.severity === IssueSeverity.MAJOR ? 'bg-rose-50 text-rose-600' :
                        issue.severity === IssueSeverity.MODERATE ? 'bg-amber-50 text-amber-700' :
                            'bg-slate-100 text-slate-500'}`}>{issue.severity}</span>
                        <span>{issue.issue}{issue.location ? ` — ${issue.location}` : ''}</span>
                      </li>))}
                  </ul>
                </div>)}
              {inspectionDetail.recommendedActions.length > 0 && (<div>
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Recommended Actions</p>
                  <ul className="space-y-1">
                    {inspectionDetail.recommendedActions.map((a, i) => (<li key={i} className="flex items-center gap-2 text-xs font-medium text-slate-600">
              ...
              <div className="flex items-center justify-between pt-2">
                ...
                {inspectionDetail.isResolved && (<span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest flex items-center gap-1">
      ...
      {deletingChecklistId && (<div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
      ...
      {deletingInspectionId && (<div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
