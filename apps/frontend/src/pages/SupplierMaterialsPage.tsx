...
export default function SupplierMaterialsPage() {
    ...
    const projectLabel = (m: MaterialAsset) => {
        ...
        if (typeof p === 'string')
            return `…${p.slice(-6)}`;
        if (p && typeof p === 'object' && p.projectName)
    ...
    return (<DashboardLayout>
      ...
      <div className="bg-white border border-slate-100 rounded-3xl overflow-hidden shadow-sm">
        ...
        {loading ? (<div className="p-12 text-center text-slate-400 text-sm">Loading…</div>) : materials.length === 0 ? (<div className="p-12 text-center">
            ...
            <table className="w-full text-left border-collapse text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {materials.map((m) => (<tr key={m._id} className="hover:bg-slate-50/50">
                    ...
                    <td className="px-6 py-4">
                      <select className="input-standard h-10 py-0 text-xs font-bold cursor-pointer min-w-[140px]" value={m.status} disabled={updatingId === m._id} onChange={(e) => handleStatusChange(m, e.target.value as MaterialStatus)}>
