...
const AuditSidebar = () => {
    ...
    useEffect(() => {
        if (!projectId)
            return;
        sustainabilityApi.getProjectMetrics(projectId, 5)
    ...
    return (<aside className="space-y-8">
      ...
      <div className="bg-surface-container-lowest p-8 rounded-3xl border border-slate-100/50 shadow-sm">
        ...
        {isLoading ? (<div className="space-y-3">
          ...
          </div>) : recentMetrics.length === 0 ? (<p className="text-xs text-slate-400 text-center py-4">No audits recorded yet</p>) : (<div className="space-y-4">
            {recentMetrics.map((metric) => (<div key={metric._id} onClick={() => navigate(`/projects/${projectId}/sustainability`)} className="flex justify-between items-center p-3 rounded-xl hover:bg-slate-50 transition-all group cursor-pointer border border-transparent hover:border-slate-100">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">
                    {new Date(metric.recordedDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: '2-digit' })}
                  </p>
                  <p className="text-xs font-bold text-primary italic">
                    {metric.scoreCategory === 'Green' ? 'Excellent' : metric.scoreCategory === 'Yellow' ? 'Good' : 'Needs Work'}
                  </p>
                </div>
        ...
        <button onClick={() => navigate(`/projects/${projectId}/sustainability`)} className="w-full mt-6 py-3 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-primary transition-all">
          View Entire History →
        </button>
      </div>
    </aside>);
    ...
    return (<DashboardLayout>
      <div className="max-w-[1600px] mx-auto">
        <header className="py-10 space-y-4">
          <Link to={`/projects/${id}/sustainability`} className="flex items-center gap-2 text-xs font-bold text-secondary uppercase tracking-widest hover:underline transition-all group">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm font-medium mt-2 max-w-2xl text-balance">
              Capture high-fidelity environmental data for your project lifecycle. These metrics are used to compute the global sustainability score and generate optimizations.
            </p>
          </div>
        </header>
