            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  <th className="text-left px-6 py-3 text-[10px] font-black uppercase tracking-widest text-slate-400">
                    User
                  </th>
                ...
                {users.map((u) => (
                  <tr key={u._id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-4 py-4">
                      <span
                        className={`text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-full ${roleBadge[u.role]}`}
                      >
                        {u.role.replace('_', ' ')}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-4">
                      <span
                        className={`text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-full ${u.isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}
                      >
            ...
            <div className="p-8 space-y-5">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  type="button"
                  onClick={() =>
                    setEditForm((f) => ({ ...f, isActive: !(f.isActive ?? editUser.isActive) }))
                  }
                ...
                >
                  <span
                    className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${(editForm.isActive ?? editUser.isActive) ? 'translate-x-5' : 'translate-x-0.5'}`}
                  />
                </button>
              </div>
          ...
          <div className="bg-white rounded-[32px] w-full max-w-sm relative shadow-2xl animate-in fade-in zoom-in duration-300 overflow-hidden">
            ...
            <div className="p-8 space-y-6">
              <p className="text-slate-600 text-sm">
                This will prevent <strong>{deactivateTarget.fullName}</strong> from logging in. You
