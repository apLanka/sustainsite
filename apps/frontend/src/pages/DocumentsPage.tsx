...
const StatusBadge = ({ status }: {
    ...
    return (<span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${sc.bg} ${sc.text}`}>
      {status}
    </span>);
};
export default function DocumentsPage() {
    ...
    const { documents, docPagination, docFilters, isDocLoading, isUploading, setDocuments, appendDocument, updateDocumentInStore, removeDocument, setDocFilters, setDocLoading, setUploading, } = useDocumentStore();
    ...
    useEffect(() => {
        ...
        const loadCounts = async () => {
            try {
                const results = await Promise.allSettled(ALL_STATUSES.map(s => documentApi.getDocuments({ projectId: docFilters.projectId, status: s, limit: 1 })));
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            if (uploadForm.tags.trim())
                form.append('tags', JSON.stringify(uploadForm.tags.split(',').map(t => t.trim()).filter(Boolean)));
    ...
    return (<DashboardLayout>
        ...
        <header className="py-10 flex flex-col md:flex-row justify-between items-end md:items-center gap-6">
          ...
          <div className="flex items-center gap-3">
            <div className="relative group">
              <input type="text" placeholder="Search by document name..." className="input-standard pl-11 pr-9 w-64 md:w-80 shadow-sm h-11" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}/>
            ...
            <select className="input-standard h-11 text-xs font-bold cursor-pointer" value={docFilters.documentType ?? ''} onChange={(e) => setDocFilters({ documentType: e.target.value as DocumentType | '', page: 1 })}>
              <option value="">All Types</option>
              {ALL_DOC_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
            </select>

            
            <select className="input-standard h-11 text-xs font-bold cursor-pointer" value={docFilters.status ?? ''} onChange={(e) => setDocFilters({ status: e.target.value as DocumentStatus | '', page: 1 })}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-10 pb-20">
          ...
          <div className="lg:col-span-1 space-y-8">
            <div onClick={() => setShowUploadModal(true)} className="cursor-pointer">
              <DocumentUploader onFileSelect={(f) => {
            setSelectedFile(f);
            if (f)
            ...
            return (<div key={s} className="flex items-center justify-between">
          ...
          <div className="lg:col-span-3">
            {isDocLoading ? (<div className="bg-surface-container-lowest rounded-3xl p-6 border border-slate-100/50 animate-pulse space-y-4">
                {Array.from({ length: 5 }).map((_, i) => (<div key={i} className="h-16 bg-slate-100 rounded-2xl"/>))}
              </div>) : visibleDocs.length === 0 ? (<div className="flex flex-col items-center justify-center py-24 text-center">
              ...
              </div>) : (<div className="bg-surface-container-lowest rounded-3xl p-2 border border-slate-100/50 shadow-sm overflow-hidden">
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {visibleDocs.map((doc) => (<tr key={doc._id} className="group hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-6 py-5 text-center">
                          ...
                          {doc.rejectionReason && (<p className="text-[9px] text-rose-400 mt-1 max-w-[120px] mx-auto line-clamp-1" title={doc.rejectionReason}>
                              {doc.rejectionReason}
                            </p>)}
                        </td>
                        ...
                        <td className="px-6 py-5 text-right">
                          <div className="flex justify-end items-center gap-1">
                            <button onClick={() => handleView(doc)} className="p-2 text-slate-400 hover:text-secondary hover:bg-slate-50 rounded-lg transition-all" title="View">
                              <span className="material-symbols-outlined text-xl">open_in_new</span>
                            </button>

                            <button onClick={() => handleDownload(doc)} className="p-2 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-all" title="Download">
                              <span className="material-symbols-outlined text-xl">download</span>
                            </button>

                            <button onClick={() => handleOpenHistory(doc)} className="p-2 text-slate-400 hover:text-secondary hover:bg-slate-50 rounded-lg transition-all" title="Version History">
                              <span className="material-symbols-outlined text-xl">history</span>
                            </button>

                            <button onClick={() => { setNewVersionDoc(doc); setShowNewVersionModal(true); }} className="p-2 text-slate-400 hover:text-primary hover:bg-slate-50 rounded-lg transition-all" title="Upload New Version">
                              <span className="material-symbols-outlined text-xl">upload</span>
                            </button>

                            {canModerate && doc.status !== DocumentStatus.APPROVED && (<button onClick={() => handleApprove(doc)} className="p-2 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-all" title="Approve">
                                <span className="material-symbols-outlined text-xl">check_circle</span>
                              </button>)}

                            {canModerate && doc.status !== DocumentStatus.REJECTED && (<button onClick={() => { setRejectingDoc(doc); setRejectionReason(''); setRejectError(null); }} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-all" title="Reject">
                                <span className="material-symbols-outlined text-xl">cancel</span>
                              </button>)}

                            <button onClick={() => setDeletingId(doc._id)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-all" title="Delete">
                ...
                {docPagination && docPagination.pages > 1 && (<div className="flex justify-center items-center gap-3 py-4 border-t border-slate-50">
                    <button className="px-4 py-2 text-xs font-bold text-slate-500 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed transition-all" disabled={docFilters.page <= 1} onClick={() => setDocFilters({ page: docFilters.page - 1 })}>
                      Previous
                    </button>
                    <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                      {docFilters.page} / {docPagination.pages}
                    </span>
                    <button className="px-4 py-2 text-xs font-bold text-slate-500 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed transition-all" disabled={docFilters.page >= docPagination.pages} onClick={() => setDocFilters({ page: docFilters.page + 1 })}>
                      Next
                    </button>
                  </div>)}
              </div>)}
      ...
      {showUploadModal && (<div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <div className="bg-white rounded-[40px] w-full max-w-2xl relative overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-300">
            ...
            <div className="p-10 space-y-6 max-h-[70vh] overflow-y-auto">
              {uploadError && (<div className="p-3 bg-rose-50 border border-rose-200 rounded-xl text-sm text-rose-600 font-medium">
                  {uploadError}
                </div>)}

              <DocumentUploader onFileSelect={(f) => setSelectedFile(f)} selectedFile={selectedFile}/>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-1.5 col-span-2">
                  ...
                  <input type="text" placeholder="e.g., Phase 2 Structural Permit" className="input-standard w-full h-12" value={uploadForm.title} onChange={(e) => setUploadForm((f) => ({ ...f, title: e.target.value }))}/>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <select className="input-standard w-full h-12 cursor-pointer" value={uploadForm.documentType} onChange={(e) => setUploadForm((f) => ({ ...f, documentType: e.target.value as DocumentType }))}>
                    {ALL_DOC_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
                  </select>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <input type="text" placeholder="1.0" className="input-standard w-full h-12" value={uploadForm.version} onChange={(e) => setUploadForm((f) => ({ ...f, version: e.target.value }))}/>
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <input type="text" placeholder="e.g., phase-1, structural, approved" className="input-standard w-full h-12" value={uploadForm.tags} onChange={(e) => setUploadForm((f) => ({ ...f, tags: e.target.value }))}/>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <textarea rows={3} placeholder="Briefly describe this document..." className="input-standard w-full resize-none" value={uploadForm.description} onChange={(e) => setUploadForm((f) => ({ ...f, description: e.target.value }))}/>
              </div>

              {isUploading && uploadProgress > 0 && (<div className="space-y-1">
              ...
              <div className="flex gap-4 pt-2">
                <button onClick={() => setShowUploadModal(false)} className="flex-1 py-4 bg-slate-100 text-primary font-bold rounded-2xl hover:bg-slate-200 transition-all cursor-pointer">
                  Cancel
                </button>
                <button onClick={handleUpload} disabled={isUploading || !selectedFile} className="flex-[2] py-4 bg-primary text-white font-black rounded-2xl shadow-xl shadow-primary/20 hover:brightness-110 active:scale-95 transition-all cursor-pointer disabled:opacity-60">
      ...
      {showHistoryModal && (<div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <div className="bg-white rounded-[40px] w-full max-w-xl relative overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-300">
            ...
            <div className="p-10 space-y-6 max-h-[60vh] overflow-y-auto">
              {isLoadingHistory ? (<div className="animate-pulse space-y-4">
                  {Array.from({ length: 3 }).map((_, i) => (<div key={i} className="h-14 bg-slate-100 rounded-xl"/>))}
                </div>) : (<>
                  
                  {historyDoc && (<div className="flex gap-4 items-start pb-6 border-b border-slate-50">
                  ...
                  {(historyDoc?.previousVersions ?? []).length === 0 ? (<p className="text-xs text-slate-400 font-medium text-center py-4">
                      No previous versions
                    </p>) : ([...(historyDoc?.previousVersions ?? [])].reverse().map((v: PreviousVersion, i) => (<div key={i} className="flex gap-4 items-start pb-6 border-b border-slate-50 last:border-0 last:pb-0">
                        ...
                        <a href={v.fileUrl} target="_blank" rel="noopener noreferrer" className="text-[10px] font-black text-secondary uppercase tracking-[0.2em] hover:underline shrink-0">
                          Download
                        </a>
                      </div>)))}
                </>)}

              <button onClick={() => setShowHistoryModal(false)} className="w-full py-4 bg-slate-100 text-primary font-bold rounded-2xl hover:bg-slate-200 transition-all cursor-pointer">
                Close
              </button>
            </div>
          </div>
      ...
      {showNewVersionModal && newVersionDoc && (<div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <div className="bg-white rounded-[40px] w-full max-w-lg relative overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-300">
            ...
            <div className="p-10 space-y-6">
              {versionError && (<div className="p-3 bg-rose-50 border border-rose-200 rounded-xl text-sm text-rose-600 font-medium">
                  {versionError}
                </div>)}
              ...
              <DocumentUploader onFileSelect={(f) => setNewVersionFile(f)} selectedFile={newVersionFile}/>
              <div className="flex gap-4 pt-2">
                <button onClick={() => { setShowNewVersionModal(false); setNewVersionFile(null); }} className="flex-1 py-4 bg-slate-100 text-primary font-bold rounded-2xl hover:bg-slate-200 transition-all cursor-pointer">
                  Cancel
                </button>
                <button onClick={handleCreateVersion} disabled={isVersioning || !newVersionFile} className="flex-[2] py-4 bg-primary text-white font-black rounded-2xl shadow-xl shadow-primary/20 hover:brightness-110 active:scale-95 transition-all cursor-pointer disabled:opacity-60">
      ...
      {rejectingDoc && (<div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <div className="bg-white rounded-[40px] w-full max-w-md relative overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-300">
            ...
            <div className="p-10 space-y-6">
              {rejectError && (<div className="p-3 bg-rose-50 border border-rose-200 rounded-xl text-sm text-rose-600 font-medium">
                  {rejectError}
                </div>)}
              <div className="space-y-1.5">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">
                ...
                <textarea rows={4} placeholder="Explain why this document is being rejected..." className="input-standard w-full resize-none" value={rejectionReason} onChange={(e) => setRejectionReason(e.target.value)}/>
              </div>
              <div className="flex gap-4 pt-2">
                <button onClick={() => setRejectingDoc(null)} className="flex-1 py-4 bg-slate-100 text-primary font-bold rounded-2xl hover:bg-slate-200 transition-all cursor-pointer">
                  Cancel
                </button>
                <button onClick={handleReject} disabled={isRejecting} className="flex-[2] py-4 bg-amber-600 text-white font-black rounded-2xl hover:brightness-110 active:scale-95 transition-all cursor-pointer disabled:opacity-60">
      ...
      {deletingId && (<div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <div className="bg-white rounded-[40px] w-full max-w-sm relative overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-300 p-10">
            ...
            <div className="flex gap-3">
              <button onClick={() => setDeletingId(null)} className="flex-1 py-3 rounded-2xl bg-slate-100 text-slate-600 text-sm font-bold hover:bg-slate-200 transition-all">
                Cancel
              </button>
              <button onClick={handleDelete} disabled={isDeleting} className="flex-1 py-3 rounded-2xl bg-rose-500 text-white text-sm font-bold hover:bg-rose-600 active:scale-95 transition-all disabled:opacity-60">
