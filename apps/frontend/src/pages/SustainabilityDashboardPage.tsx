...
import {
  ...
  ImpactRadar,
  ResourceBarChart,
} from '@/components/sustainability/SustainabilityCharts';
...
export default function SustainabilityDashboardPage() {
    ...
    ? [
        {
          ...
          A: Math.max(
            0,
            Math.min(100, Math.round(100 - (latestMetric.carbonEmissions.total / 10) * 100))
          ),
          fullMark: 100,
        },
        {
          subject: 'Energy',
          A:
            latestMetric.energyConsumption.total > 0
              ? Math.round(
                  (latestMetric.energyConsumption.renewableEnergy /
                    latestMetric.energyConsumption.total) *
                    100
                )
              : 0,
          fullMark: 100,
        ...
        { subject: 'Waste', A: latestMetric.wasteManagement.diversionRate, fullMark: 100 },
        {
          subject: 'Water',
          A:
            latestMetric.waterUsage.total > 0
  ...
  const lineData =
    trends.length >= 1
      ? trends
          ...
          .map((t, i) => ({
            name: t.recordedDate
              ? new Date(t.recordedDate).toLocaleDateString('en-US', {
                  month: 'short',
                  day: 'numeric',
                })
              : `Entry ${i + 1}`,
  ...
  const barData = latestMetric
    ? [
        ...
        {
          name: 'Waste Div.',
          value: Math.round(latestMetric.wasteManagement.diversionRate),
          color: '#059669',
        },
      ]
    : undefined;
        ...
        <div className="py-10 flex flex-col md:flex-row justify-between items-end md:items-center gap-6">
          <div>
            ...
            <p className="text-slate-500 text-sm font-medium mt-2">
              Analytical breakdown of site-specific ecological vitals.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            ...
            <div className="flex flex-col gap-10 min-h-[480px]">
              <div className="bg-gradient-to-br from-emerald-950 to-primary p-10 rounded-3xl shadow-xl shadow-emerald-950/20 text-white relative overflow-hidden group flex-1 flex flex-col justify-center">
                ...
                <p className="text-[10px] font-black uppercase tracking-[0.2em] mb-4 text-emerald-100/80">
                  Net Carbon Sequestration
                </p>
                <h3 className="text-5xl font-extrabold tracking-tighter mb-2">
                  {treesEquivalent.toLocaleString()}
                </h3>
                <p className="text-sm font-medium text-emerald-200/80">
                  Project Trees Equivalent Offset
                </p>
              ...
              <div className="bg-surface-container-lowest p-10 rounded-3xl border border-slate-100/50 shadow-sm flex-1 flex flex-col justify-center">
                ...
                <div className="flex items-end gap-3">
                  ...
                  <span className="text-sm font-bold text-slate-400 mb-1 leading-none uppercase tracking-widest font-headline">
                    tCO2e / sq.m
                  </span>
                </div>
                <div className="mt-8 flex gap-1.5 h-2">
                  {[1, 2, 3, 4, 5, 6].map((i) => (
                    <div
                      key={i}
                      className={`flex-1 rounded-full ${i <= filledSegments ? 'bg-secondary shadow-[0_0_8px_rgba(14,108,74,0.3)]' : 'bg-slate-100'}`}
                    ></div>
                  ))}
                </div>
                <p className="text-[10px] font-bold text-slate-400 mt-4 uppercase tracking-[0.1em]">
                  {industryLabel}
                </p>
              </div>
            </div>
