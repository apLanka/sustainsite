...
import type { IndustryComparisonResponse, SustainabilityMetric, SustainabilityScore as ScoreType, SustainabilityTrendsResponse, } from '@/types/sustainability';
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (latestRes.status === 'fulfilled') {
                    ...
                    const status = (latestRes.reason as {
                        status?: number;
                    })?.status;
                    if (status !== 404) {
                ...
                const latestIs404 = latestRes.status === 'rejected' &&
                    (latestRes.reason as {
                        status?: number;
                    })?.status === 404;
                const allFailed = scoreRes.status === 'rejected' &&
      ...
      <div className="px-10">
        ...
        <div className="py-10 flex flex-col md:flex-row justify-between items-end md:items-center gap-6">
          ...
          <div className="flex items-center gap-4">
            <button onClick={handleExport} className="px-6 py-2.5 bg-surface-container-high text-primary font-bold text-sm rounded-xl flex items-center justify-center gap-2 hover:bg-slate-200 transition-all cursor-pointer font-headline">
              <span className="material-symbols-outlined text-lg">download</span>
              Export Report
            </button>
            {canRecord && (<Link to={`/projects/${projectId}/sustainability/record`} className="px-6 py-2.5 bg-primary text-white font-bold text-sm rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-primary/10 hover:brightness-110 active:scale-95 transition-all cursor-pointer font-headline">
        ...
        <div className="space-y-10 pb-20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <div className="lg:col-span-1 min-h-[440px]">
              {isLoading ? (<div className="bg-emerald-950 rounded-3xl animate-pulse h-full min-h-[440px]"/>) : error ? (<div className="bg-emerald-950 rounded-3xl p-8 flex items-center justify-center h-full">
                    <p className="text-emerald-400 text-sm font-medium">{error}</p>
                  </div>) : (<SustainabilityScore score={sustainabilityScoreValue} trend={scoreData?.trend} scoreCategory={scoreData?.scoreCategory}/>)}
            </div>
            <div className="lg:col-span-2 bg-surface-container-lowest p-10 rounded-3xl border border-slate-100/50 shadow-sm flex flex-col justify-center">
              ...
              {isLoading ? (<div className="h-80 animate-pulse bg-slate-100 rounded-2xl"/>) : lineData ? (<div className="h-80">
                  <HistoricalTrendLine data={lineData}/>
                </div>) : (<div className="h-80 flex flex-col items-center justify-center text-slate-400 gap-3">
                  ...
                  {canRecord && (<Link to={`/projects/${projectId}/sustainability/record`} className="text-xs text-emerald-600 font-bold hover:underline">
            ...
            <div className="flex flex-col gap-10 min-h-[480px]">
                ...
                <div className="bg-surface-container-lowest p-10 rounded-3xl border border-slate-100/50 shadow-sm flex-1 flex flex-col justify-center">
                  ...
                  <div className="mt-8 flex gap-1.5 h-2">
                    {[1, 2, 3, 4, 5, 6].map(i => (<div key={i} className={`flex-1 rounded-full ${i <= filledSegments ? 'bg-secondary shadow-[0_0_8px_rgba(14,108,74,0.3)]' : 'bg-slate-100'}`}></div>))}
                  </div>
