...
export const updateChecklist = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (lastReviewDate !== undefined)
            checklist.lastReviewDate = lastReviewDate;
        if (items !== undefined) {
...
export const createInspection = async (req: Request, res: Response): Promise<void> => {
    try {
        const { projectId, inspectionType, inspectionDate, inspectorNotes, findings, riskLevel, issuesIdentified, actionRequired, recommendedActions, actionDeadline, attachments, photos, followUpDate, followUpNotes, } = req.body;
...
export const getInspections = async (req: Request, res: Response): Promise<void> => {
    try {
        const { projectId, riskLevel, actionStatus, isResolved, inspectionType, page = '1', limit = '10', } = req.query;
        const filter: Record<string, unknown> = {};
        if (projectId) {
            if (!mongoose.Types.ObjectId.isValid(projectId as string)) {
                res.status(400).json({ success: false, error: 'Invalid projectId format' });
...
export const updateInspection = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const { inspectionType, inspectionDate, inspectorNotes, findings, riskLevel, issuesIdentified, actionRequired, recommendedActions, actionDeadline, actionStatus, isResolved, attachments, photos, followUpDate, followUpNotes, } = req.body;
        ...
        if (inspectorNotes !== undefined)
            inspection.inspectorNotes = inspectorNotes;
        if (findings !== undefined)
            inspection.findings = findings;
        if (riskLevel !== undefined)
            inspection.riskLevel = riskLevel;
        if (issuesIdentified !== undefined)
            inspection.issuesIdentified = issuesIdentified;
        if (actionRequired !== undefined)
            inspection.actionRequired = actionRequired;
        if (recommendedActions !== undefined)
            inspection.recommendedActions = recommendedActions;
        if (actionDeadline !== undefined)
            inspection.actionDeadline = actionDeadline;
        if (actionStatus !== undefined)
            inspection.actionStatus = actionStatus;
        if (isResolved !== undefined)
            inspection.isResolved = isResolved;
        ...
        if (followUpNotes !== undefined)
            inspection.followUpNotes = followUpNotes;
