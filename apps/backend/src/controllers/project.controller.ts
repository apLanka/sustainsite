...
export const createProject = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const userId = (req as unknown as {
            user: {
                userId: string;
            };
        }).user.userId;
...
export const getProjects = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const sort: ListSort = isTextSearch
...
export const updateMilestone = async (req: Request, res: Response): Promise<void> => {
    try {
        const milestone = await Milestone.findOneAndUpdate({ _id: req.params.milestoneId, projectId: req.params.id }, { $set: req.body }, { new: true, runValidators: true });
        if (!milestone) {
            res.status(404).json({
                success: false,
...
export const getFinancialSummary = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const remainingValue = materials.reduce((sum, mat) => sum + (Number(mat.currentStock) || 0) * (Number(mat.unitPrice) || 0), 0);
        ...
        const spendPercentage = budget > 0 && Number.isFinite(totalSpendSafe) ? (totalSpendSafe / budget) * 100 : 0;
        res.status(200).json({
            success: true,
            data: {
                projectId: id,
