...
export const updateMetric = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (req.body.waterUsage)
            metric.waterUsage = { ...metric.waterUsage, ...req.body.waterUsage };
        if (req.body.notes !== undefined)
            metric.notes = req.body.notes;
        await metric.save();
        const project = await Project.findById(metric.projectId);
        if (project) {
            project.sustainabilityScore = metric.sustainabilityScore;
    ...
    try {
        ...
        res.status(200).json({
            ...
            data: {
                ...
                scoreCategory: project.sustainabilityScore >= 75 ? 'Green'
...
export const compareWithIndustry = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (score >= INDUSTRY_BENCHMARKS.excellent)
            percentileBand = 'Top 15% (Excellent)';
        else if (score >= INDUSTRY_BENCHMARKS.good)
            percentileBand = 'Top 35% (Good)';
        else if (score >= INDUSTRY_BENCHMARKS.average)
            percentileBand = 'Above average';
        else
            percentileBand = 'Below average';
        ...
        if (latest) {
            ...
            if (latest.carbonEmissions.total < INDUSTRY_BENCHMARKS.carbonAvgTonsPerWeek)
                ...
                areasBelowAverage.push('Carbon emissions');
            if (renewablePct > 20)
                areasAboveAverage.push('Renewable energy usage');
            else
                areasBelowAverage.push('Renewable energy usage');
            if (latest.wasteManagement.diversionRate > 60)
                areasAboveAverage.push('Waste diversion rate');
            else
                areasBelowAverage.push('Waste diversion rate');
            const totalWater = latest.waterUsage.municipal + latest.waterUsage.recycled;
            const recycledWaterPct = totalWater > 0 ? (latest.waterUsage.recycled / totalWater) * 100 : 0;
            if (recycledWaterPct > 20)
                areasAboveAverage.push('Water recycling');
            else
                areasBelowAverage.push('Water recycling');
        }
        res.status(200).json({
            success: true,
            data: {
