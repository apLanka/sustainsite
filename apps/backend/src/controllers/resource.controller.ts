            ...
            try {
                ...
                if (supplierDoc?.email && projectDoc) {
                    await sendEmail({
                        ...
                        html: emailTemplates.purchaseOrder(materialName, quantity, unit, projectDoc.projectName, expectedDeliveryDate ? new Date(expectedDeliveryDate).toDateString() : 'TBD'),
                    });
                }
            }
            catch (emailErr) {
                ...
                if (project?.projectManager) {
                    ...
                    if (manager?.email) {
                        await sendEmail({
                            ...
                            html: emailTemplates.lowStockAlert(material.materialName, material.currentStock, material.minimumThreshold, material.unit),
                        });
                    }
                }
            }
