...
export const uploadDocument = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!projectId || !documentType || !title) {
      ...
      res
        .status(400)
        .json({ success: false, error: 'projectId, documentType, and title are required' });
      return;
    }
    ...
    const { url, cloudinaryId, format, size } = await uploadToCloudinary(
      req.file.path,
      'construction-docs'
    );
    fs.unlink(req.file.path, () => {});
...
export const updateDocumentStatus = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (
      ...
      res
        .status(403)
        .json({ success: false, error: 'Only ADMIN or INSPECTOR can approve or reject documents' });
      return;
    }
    ...
    if (status === DocumentStatus.APPROVED) {
      ...
      if (!rejectionReason) {
        res
          .status(400)
          .json({ success: false, error: 'rejectionReason is required when rejecting' });
        return;
      }
