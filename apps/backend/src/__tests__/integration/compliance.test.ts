...
import SafetyInspection, { RiskLevel, ActionStatus, InspectionType, IssueSeverity } from '../../models/SafetyInspection';
...
describe('Compliance Management API', () => {
    ...
    beforeEach(async () => {
        ...
        const inspector = await createTestUser({ email: 'comp-inspector@test.com', role: UserRole.INSPECTOR });
    ...
    describe('GET /api/compliance/checklists', () => {
        beforeEach(async () => {
            await seedChecklist({ checklistName: 'Safety Checklist', category: ComplianceCategory.SAFETY });
            await seedChecklist({ checklistName: 'Environmental Checklist', category: ComplianceCategory.ENVIRONMENTAL });
        });
        ...
        it('should filter by projectId', async () => {
            await seedChecklist({ projectId: new mongoose.Types.ObjectId().toString(), checklistName: 'Other Project' });
            const response = await request(app)
    ...
    describe('POST /api/compliance/inspections', () => {
        it('should create an inspection as INSPECTOR', async () => {
            const response = await request(app)
                ...
                riskLevel: RiskLevel.HIGH,
                issuesIdentified: [{ issue: 'No harnesses', severity: IssueSeverity.MAJOR, location: 'Level 3' }],
    ...
    describe('GET /api/compliance/inspections', () => {
        beforeEach(async () => {
            ...
            await seedInspection({ riskLevel: RiskLevel.LOW, actionStatus: ActionStatus.COMPLETED, isResolved: true });
        });
    ...
    describe('PUT /api/compliance/checklists/:id/items/:itemId', () => {
        ...
        it('should update item notes without changing completion status', async () => {
            const response = await request(app)
                ...
                .expect(200);
            expect(response.body.success).toBe(true);
            const item = response.body.data.items.find((i: {
                itemId: string;
            }) => i.itemId === 'item-1');
