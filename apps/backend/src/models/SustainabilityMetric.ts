...
sustainabilityMetricSchema.pre('save', async function () {
    ...
    const energyScore = calculateEnergyScore(this.energyConsumption.electricity, this.energyConsumption.renewableEnergy);
    ...
    this.sustainabilityScore = Math.round(carbonScore * 0.3 + energyScore * 0.25 + wasteScore * 0.25 + waterScore * 0.2);
...
const SustainabilityMetric: Model<ISustainabilityMetric> = mongoose.model<ISustainabilityMetric>('SustainabilityMetric', sustainabilityMetricSchema);
