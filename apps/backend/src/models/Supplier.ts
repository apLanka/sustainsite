...
supplierSchema.methods.addRating = async function (ratedBy: mongoose.Types.ObjectId, rating: number, comment?: string): Promise<ISupplier> {
    this.ratings.push({
        ratedBy,
        rating,
        comment,
...
supplierSchema.methods.updatePerformanceMetrics = async function (onTimeDelivery: boolean): Promise<ISupplier> {
    this.totalOrders += 1;
